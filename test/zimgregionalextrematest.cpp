...
TEST(ZImgRegionalExtrema, max)
{
  ...
  for (ZImgRegionIterator<uint8_t> it = ZImgRegionIterator<uint8_t>(img, rgn1); !it.isAtEnd(); ++it) {
    *it = 22;
  }
  ZImgRegion rgn2(5, 8, 5, 8);
  for (ZImgRegionIterator<uint8_t> it = ZImgRegionIterator<uint8_t>(img, rgn2); !it.isAtEnd(); ++it) {
    *it = 33;
  }
  ...
  for (ZImgRegionConstIterator<uint8_t> it = ZImgRegionConstIterator<uint8_t>(mask); !it.isAtEnd(); ++it) {
    ...
    if (rgn1.containsCoord(coord, info) || rgn2.containsCoord(coord, info) || (coord.x == 2 && coord.y == 7)) {
      ASSERT_EQ(1, *it) << fmt::format("{}", coord);
    } else {
      ASSERT_EQ(0, *it) << fmt::format("{}", coord);
    }
  }
}
