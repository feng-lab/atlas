...
{
  std::vector<int> data;
  EXPECT_EQ(parallel_max_element(data.begin(), data.end()), data.end());
...
TEST(ParallelTest, TestParallelMeanAndVarianceWithInts)
{
  std::vector<int> data(10000);
  std::iota(data.begin(), data.end(), 1);
  auto result = parallel_mean_and_variance(data.begin(), data.end());
...
TEST(ParallelTest, TestParallelMeanAndSampleVarianceWithInts)
{
  std::vector<int> data(10000);
  std::iota(data.begin(), data.end(), 1);
  auto result = parallel_mean_and_sample_variance(data.begin(), data.end());
  EXPECT_NEAR(result.first, 5000.5, 1e-5); // the exact mean is 5000.5
...
TEST(ParallelTest, TestParallelMeanAndStandardDeviationWithInts)
{
  std::vector<int> data(10000);
  std::iota(data.begin(), data.end(), 1);
  auto result = parallel_mean_and_standard_deviation(data.begin(), data.end());
  EXPECT_NEAR(result.first, 5000.5, 1e-5); // the exact mean is 5000.5
...
TEST(ParallelTest, TestParallelMeanAndSampleStandardDeviationWithInts)
{
  std::vector<int> data(10000);
  std::iota(data.begin(), data.end(), 1);
  auto result = parallel_mean_and_sample_standard_deviation(data.begin(), data.end());
  EXPECT_NEAR(result.first, 5000.5, 1e-5); // the exact mean is 5000.5
