...
TEST(ImgIterator, region)
{
  ...
  ZImg img(info);
  auto data = img.timeData<uint32_t>(0);
  ...
  int64_t idx = 0;
  for (ZImgRegionConstIterator<uint32_t> it = ZImgRegionConstIterator<uint32_t>(img, region); !it.isAtEnd(); ++it) {
    if (idx == 0) {
...
TEST(ImgIterator, neighborhoodWithPtr)
{
  ...
  region.resolveRegionEnd(info);
  int64_t idx = 0;
  ZImgNeighborhoodWithPtrConstIterator<uint32_t> it =
    ZImgNeighborhoodWithPtrConstIterator<uint32_t>(ZNeighborhood(18), img, region);
  for (; !it.isAtEnd(); ++it) {
    if (idx == 0) {
      ASSERT_EQ(*img.data<uint32_t>(region.start), *it);
    } else if (idx == 999) {
    ...
    ASSERT_EQ(ZVoxelCoordinate(0, 0, 1), coord - nb1) << fmt::format("{} {}", coord, nb1);
    ASSERT_EQ(10000_u32, *it - it.valueRef(2))
      << *it << " " << it.valueRef(2) << fmt::format("{} {}", coord, nb1) << (&(*it) - it.valuePtr(2));
    if (coord.z == 19) {
      for (size_t i = 0; i < it.numNeighbors(); ++i) {
        if (i < 13) {
          ASSERT_TRUE(it.isInBound(i));
...
TEST(ImgIterator, neighborhoodWithCoord)
{
  ...
  region.resolveRegionEnd(info);
  int64_t idx = 0;
  ZImgNeighborhoodWithCoordConstIterator<uint32_t> it =
    ZImgNeighborhoodWithCoordConstIterator<uint32_t>(ZNeighborhood(18), img, region);
  for (; !it.isAtEnd(); ++it) {
    if (idx == 0) {
      ASSERT_EQ(*img.data<uint32_t>(region.start), *it);
    } else if (idx == 999) {
