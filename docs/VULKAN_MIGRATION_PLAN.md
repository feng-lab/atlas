...
- **Status (2025-10-08):** Vulkan frames now stay inside a single command buffer per eye: `Z3DRendererBase::beginVulkanFrame`/`recordVulkanBatches` gate command recording so the compositor only submits once at `endVulkanFrame` (`src/atlas/z3drendererbase.cpp#L354`, `src/atlas/z3dcompositor.cpp#L1237`, `src/atlas/z3dcompositor.cpp#L1821`). `Z3DRendererVulkanBackend` no longer owns per-frame command pools; instead it borrows `ZVulkanFrameExecutor::ActiveFrame` instances (command buffer, fence, and acquire/release semaphores) for every render call (`src/atlas/z3drenderervulkanbackend.cpp`). `ZVulkanSwapChain` now submits via the same executor, signalling the shared release semaphore and exposing both sync points for presentation wiring (`src/atlas/zvulkanswapchain.cpp`). **Next:** implement the fence-failure regression described below and thread the release semaphore through the presentation path once the window-system swapchain lands.
