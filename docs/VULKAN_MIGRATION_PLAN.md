...
- Cones (cone_2) front-face: Vulkan considers the resulting bounding triangles CW under our clip/viewport conventions. Pipelines now set `frontFace=Clockwise` so `gl_FrontFacing` in the cone fragment path matches GL; culling remains disabled and the early backface discard in the fragment keeps rasterized quads minimal.
- Depth mapping (Vulkan): For impostor primitives (spheres, ellipsoids, cones), fragment depth now uses Vulkan NDC directly: `depth = (Pz*P[2].w + P[3].w) / (Pz*P[2].z + P[3].z)` simplified as `clipZW.x/clipZW.y` from the projection matrix rows. The previous GL-style conversion (`0.5 + 0.5*z_ndc`) caused fragments to fail depth tests on Vulkan (z in [0,1]) and could make primitives invisible.
