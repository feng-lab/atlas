...
- Integration point is `ZImgPack`: 2D uses on-demand chunk reads for the visible viewport; 3D paging uses `ZImgPack::readRegionToImgAsync()` to populate the GPU page cache. Synchronous “hot-path” queries (`displayValue()`/`value()`) are cache-only to avoid blocking UI/render threads on network I/O.
  ...
  - Default preview/final 2D rendering still uses the existing per-chunk fan-out path so tiles stream in progressively and keep the established interaction quality.
  - `--atlas_ng_precomputed_use_batched_chunk_reads=true` switches 2D preview/final passes and synchronous Neuroglancer pack helpers to the experimental batched path built on `ZNeuroglancerPrecomputedVolume::sliceTilePackFor2DViewportAsync(...)` plus bounded `collectAllWindowed(...)`. Use that flag for crash-isolation experiments; keep the default path unchanged unless the experiment proves out.
