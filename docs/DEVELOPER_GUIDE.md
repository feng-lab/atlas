...
- Block-ID pass and Full-Res Cache
  ...
  - Paged-image page-directory and page-table metadata uploads are demand driven: texture accessors restore metadata when backend textures are missing or invalidated, and `Z3DImg::updateAndUploadPageDirectoryCaches()` uploads after actual page-table changes. Small page-table updates upload only dirty 32x32x32 page-table cache blocks; larger updates fall back to a full metadata upload to avoid many immediate submissions. Use `--atlas_dirty_page_table_upload_max_blocks=N` to tune this crossover for exports and benchmarks: `0` forces full page-table uploads after paging changes, positive values allow partial upload up to `N` dirty blocks, and negative values disable the count-based full-upload fallback. Render prep must not reupload the full metadata textures every pass because page-table caches can be hundreds of MiB.
