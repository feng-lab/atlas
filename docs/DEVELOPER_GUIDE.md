...
- Caches (per opened Neuroglancer volume; these are budgets, not preallocated):
  ...
  - Default preview/final 2D rendering is platform-dependent for now: Windows defaults to the batched chunk-read path because it avoids the reproducible Windows-only heap-corruption/crash family seen under the older fan-out path, while macOS/Linux still default to the existing per-chunk fan-out path because it streams tiles more smoothly.
  - `--atlas_ng_precomputed_use_batched_chunk_reads=true|false` overrides that platform default for both 2D preview/final passes and the synchronous Neuroglancer pack helpers. The batched path is built on `ZNeuroglancerPrecomputedVolume::sliceTilePackFor2DViewportAsync(...)` plus bounded `collectAllWindowed(...)`; the legacy path still uses per-chunk fan-out.
