...
- Vulkan: pipeline contexts create 2D Nx1 `ZVulkanTexture`s via a small helper (MoltenVK portability — Metal lacks native 1D); LUTs are uploaded as RGBA8, registered in the bindless sampled-image tables (set=0), and referenced by index from shaders (push constants / UBOs).
  ...
  - Vulkan scratch reuse policy: completed scratch slots are reusable slot inventory, not correctness state. The first-order goal is reuse parity with OpenGL: Vulkan should reuse compatible free scratch slots before allocating new ones, including reusing larger BlockID attachment sets when the request needs fewer attachments. If no free slot is visible but a compatible slot has a deferred release pending, acquisition pumps the completion safe point and rescans before growing.
...
- Vulkan residency broker (`ZVulkanResidencyManager`):
  ...
  - Reclaim order is intentionally conservative and cost-aware: poll/drain safe points at the caller boundary, release completed upload pages, release free scratch backing while keeping slot/texture identity, release host-backed cold scratch backing that is not in the next pass hot set, trim free readback staging, demote cold static geometry streams, evict unpinned paged image caches, then evict unpinned dense volumes. Waiting for all in-flight work is a stronger caller-selected mode, not something arbitrary allocation sites do mid-recording.
  - Managed texture classes include paged R8 `imageCache` textures, page directory / page table metadata textures, and dense R8 3D preview volumes. Dense volumes restore from the owning `Z3DImg` channel image/generation; paged image caches restore from host backup when dirty or from a deterministic zero clear when untouched; page metadata textures restore from the owning `Z3DImg` cache views.
  - Any texture/buffer used by a submitted command buffer must be pinned or otherwise protected until the submission fence reaches the backend completion safe point. Dense volumes and paged image caches are pinned by the image pipeline contexts; static arena segments are pinned by static slices; scratch backings are protected by pass hot-set residency and may be evicted only after the safe point.
