...
- Caches (per opened Neuroglancer volume; these are budgets, not preallocated):
  ...
  - Default preview/final 2D rendering now uses the non-batched coroutine fan-out path on all platforms. The batched chunk-read path remains available as a fallback for A/B testing and incident response.
