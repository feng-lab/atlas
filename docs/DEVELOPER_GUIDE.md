...
- Vulkan: pipeline contexts create 2D Nx1 `ZVulkanTexture`s via a small helper (MoltenVK portability — Metal lacks native 1D); LUTs are uploaded as RGBA8, registered in the bindless sampled-image tables (set=0), and referenced by index from shaders (push constants / UBOs).
  ...
  - Vulkan scratch reuse policy: completed scratch slots are reusable slot inventory, not correctness state. The first-order goal is reuse parity with OpenGL: Vulkan should reuse compatible free scratch slots before allocating new ones, including reusing larger BlockID attachment sets when the request needs fewer attachments. If no free slot is visible but a compatible slot has a deferred release pending, acquisition pumps the completion safe point and rescans before growing. Normal reclaim pumps frame-completion safe points so deferred Vulkan releases become reusable; it does not proactively release resident scratch backing. The allocation-failure retry path can wait for in-flight work and release free resident scratch backing memory, but it must not destroy bindless-registered scratch texture objects. This free-backing release is a last-resort recovery path, not the normal reclaim policy.
  - Non-progressive capture/export does not wait between Vulkan filters just to trim scratch. Persistent filter outputs remain leased for the compositor, and reusable scratch texture objects are preserved. If memory pressure occurs, allocation-failure recovery evicts free backing memory and retries while preserving scratch slot and `ZVulkanTexture` identity.
...
- Vulkan residency: Vulkan `trim()` and allocation-failure recovery release only the free slot's backing `VkImage` allocations and keep the `ZVulkanTexture` objects alive. When a slot is reused, the same texture object is made resident again or retargeted in place, so bindless descriptor entries are rewritten at their existing indices instead of being consumed by residency changes.
