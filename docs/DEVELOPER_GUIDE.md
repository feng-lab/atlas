...
- Atlas can load Neuroglancer “precomputed” volumes over HTTP (both unsharded and sharded storage via `"sharding": {"@type":"neuroglancer_uint64_sharded_v1", ...}`) with these chunk encodings:
  ...
  - Networking is routed through `src/atlas/zhttpclient.h` and `src/atlas/zhttpclient.cpp`, with the runtime backend selected by `--atlas_http_backend=proxygen|curl`.
    - `proxygen`: `src/atlas/zproxygenhttpclient.h` and `src/atlas/zproxygenhttpclient.cpp`.
      ...
      - The external Proxygen build is patched in `util/build_ext_libs.py` in two Windows-targeted places derived from dump analysis: direct `HTTPCoroConnector` coroutines actively cancel in-flight connect callbacks before unwinding on coroutine cancellation, and shared HTTP CONNECT streams retain a keepalive on the underlying proxy session for the full tunnel lifetime. These are targeted mitigations for Windows crash manifestations we observed in practice, not claims of a fully proven cross-platform root cause.
...
- Caches (per opened Neuroglancer volume; these are budgets, not preallocated):
  ...
  - Default preview/final 2D rendering is platform-dependent for now: Windows defaults to the batched chunk-read path because it avoids the reproducible Windows-only heap-corruption/crash family seen under the older fan-out path, while macOS/Linux still default to the non-batched path.
  - `--atlas_ng_precomputed_use_batched_chunk_reads=true|false` still selects the high-level mode for both 2D preview/final passes and the synchronous Neuroglancer pack helpers. The batched path is built on `ZNeuroglancerPrecomputedVolume::sliceTilePackFor2DViewportAsync(...)` plus bounded `collectAllWindowed(...)`; the non-batched path now always uses Folly coroutine fan-out so tiles still stream progressively as they finish.
