...
| `--atlas_bioformats_bridge_worker_count` | Bio-Formats gRPC reader count for pixel reads. Default `0` auto-selects the CPU thread count for Java-side reader workers inside one JVM. |
