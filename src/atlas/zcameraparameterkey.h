...
public:
  ...
  float posContinuity() const { return m_posContinuity; }
  void setPosContinuity(float v) { m_posContinuity = std::min(std::max(v,-1.f), 1.f); }

  float posBias() const { return m_posBias; }
  void setPosBias(float v) { m_posBias = std::min(std::max(v,-1.f), 1.f); }

  float rotTension() const { return m_rotTension; }
  void setRotTension(float v) { m_rotTension = std::min(std::max(v,-1.f), 1.f); }

  float rotContinuity() const { return m_rotContinuity; }
  void setRotContinuity(float v) { m_rotContinuity = std::min(std::max(v,-1.f), 1.f); }

  float rotBias() const { return m_rotBias; }
  void setRotBias(float v) { m_rotBias = std::min(std::max(v,-1.f), 1.f); }
