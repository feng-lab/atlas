...
struct RegionNode
{
  explicit RegionNode(int64_t id_ = -1,
                      int64_t parentID_ = -2,
                      int red_ = 255,
                      int green_ = 255,
                      int blue_ = 255,
                      QString name_ = "Undefined",
                      QString abbreviation_ = "Undefined")
    : id(id_)
    , parentID(parentID_)
    , red(red_)
    , green(green_)
    , blue(blue_)
    , name(std::move(name_))
    , abbreviation(std::move(abbreviation_))
  {}
...
// input img, an unsigned 8-bit single-channel image. Pixels with value 1 is treated as foreground. Other pixels are
// background.
