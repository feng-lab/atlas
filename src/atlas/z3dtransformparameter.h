...
class Z3DTransformParameter : public ZSingleValueParameter<glm::mat4>
{
  Q_OBJECT
public:
  Z3DTransformParameter(const QString& name, QObject* parent = NULL);
  Z3DTransformParameter(const QString& name, const glm::mat4& value, QObject* parent = NULL);

  inline void setScale(const glm::vec3& v) { m_scale.set(v); }
  inline void setTranslation(const glm::vec3& v) { m_translation.set(v); }
  inline void setRotationCenter(const glm::vec3 &c) { m_center.set(c); }
  // angle in degree and axis
  inline void setRotation(const glm::vec4& v) { m_rotation.set(v); }
  inline void setRotation(const glm::quat& v) { m_rotation.set(glm::vec4(glm::degrees(glm::angle(v)), glm::axis(v))); }
  inline void setCenter(const glm::vec3& v) { return m_center.set(v); }
  inline glm::vec3 scale() const { return m_scale.get(); }
  inline glm::vec3 translation() const { return m_translation.get(); }
  glm::quat rotation() const;
  inline void setXScale(float s) { m_scale.set(glm::vec3(s, m_scale.get().y, m_scale.get().z)); }
  inline void setYScale(float s) { m_scale.set(glm::vec3(m_scale.get().x, s, m_scale.get().z)); }
  inline void setZScale(float s) { m_scale.set(glm::vec3(m_scale.get().x, m_scale.get().y, s)); }
