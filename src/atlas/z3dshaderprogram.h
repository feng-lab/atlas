...
public:
  ...
  {
    if (loc != -1) {
      glUniformMatrix2fv(loc, 1, GLboolean(transpose), &value[0][0]);
    }
  }
  ...
  {
    if (loc != -1) {
      glUniformMatrix3fv(loc, 1, GLboolean(transpose), &value[0][0]);
    }
  }
  ...
  {
    if (loc != -1) {
      glUniformMatrix4fv(loc, 1, GLboolean(transpose), &value[0][0]);
    }
  }
  ...
  {
    if (loc != -1) {
      glUniformMatrix2fv(loc, count, GLboolean(transpose), &(values[0][0][0]));
    }
  }
  ...
  {
    if (loc != -1) {
      glUniformMatrix3fv(loc, count, GLboolean(transpose), &(values[0][0][0]));
    }
  }
  ...
  {
    if (loc != -1) {
      glUniformMatrix4fv(loc, count, GLboolean(transpose), &(values[0][0][0]));
    }
  }
  ...
  {
    if (m_screenDimUniform) {
      setUniform(m_screenDimUniform->location, value);
    }
  }

  void setScreenDimRCPUniform(const glm::vec2& value)
  {
    if (m_screenDimRCPUniform) {
      setUniform(m_screenDimRCPUniform->location, value);
    }
  }

  void setCameraPositionUniform(const glm::vec3& value)
  {
    if (m_cameraPositionUniform) {
      setUniform(m_cameraPositionUniform->location, value);
    }
  }

  void setViewMatrixUniform(const glm::mat4& value)
  {
    if (m_viewMatrixUniform) {
      setUniform(m_viewMatrixUniform->location, value);
    }
  }

  void setViewMatrixInverseUniform(const glm::mat4& value)
  {
    if (m_viewMatrixInverseUniform) {
      setUniform(m_viewMatrixInverseUniform->location, value);
    }
  }

  void setProjectionMatrixUniform(const glm::mat4& value)
  {
    if (m_projectionMatrixUniform) {
      setUniform(m_projectionMatrixUniform->location, value);
    }
  }

  void setProjectionMatrixInverseUniform(const glm::mat4& value)
  {
    if (m_projectionMatrixInverseUniform) {
      setUniform(m_projectionMatrixInverseUniform->location, value);
    }
  }

  void setNormalMatrixUniform(const glm::mat3& value)
  {
    if (m_normalMatrixUniform) {
      setUniform(m_normalMatrixUniform->location, value);
    }
  }

  void setViewportMatrixUniform(const glm::mat4& value)
  {
    if (m_viewportMatrixUniform) {
      setUniform(m_viewportMatrixUniform->location, value);
    }
  }

  void setViewportMatrixInverseUniform(const glm::mat4& value)
  {
    if (m_viewportMatrixInverseUniform) {
      setUniform(m_viewportMatrixInverseUniform->location, value);
    }
  }

  void setProjectionViewMatrixUniform(const glm::mat4& value)
  {
    if (m_projectionViewMatrixUniform) {
      setUniform(m_projectionViewMatrixUniform->location, value);
    }
  }
  ...
  {
    if (m_gammaUniform) {
      setUniform(m_gammaUniform->location, value);
    }
  }

  void setSizeScaleUniform(float value)
  {
    if (m_sizeScaleUniform) {
      setUniform(m_sizeScaleUniform->location, value);
    }
  }

  void setPosTransformUniform(const glm::mat4& value)
  {
    if (m_posTransformUniform) {
      setUniform(m_posTransformUniform->location, value);
    }
  }

  void setPosTransformNormalMatrixUniform(const glm::mat3& value)
  {
    if (m_posTransformNormalMatrixUniform) {
      setUniform(m_posTransformNormalMatrixUniform->location, value);
    }
  }
  ...
  void setLightsPositionUniform(const glm::vec4* value, int count)
  {
    if (m_lightsPositionUniform) {
      setUniformArray(m_lightsPositionUniform->location, value, count);
    }
  }

  void setLightsAmbientUniform(const glm::vec4* value, int count)
  {
    if (m_lightsAmbientUniform) {
      setUniformArray(m_lightsAmbientUniform->location, value, count);
    }
  }

  void setLightsDiffuseUniform(const glm::vec4* value, int count)
  {
    if (m_lightsDiffuseUniform) {
      setUniformArray(m_lightsDiffuseUniform->location, value, count);
    }
  }

  void setLightsSpecularUniform(const glm::vec4* value, int count)
  {
    if (m_lightsSpecularUniform) {
      setUniformArray(m_lightsSpecularUniform->location, value, count);
    }
  }

  void setLightsSpotCutoffUniform(const float* value, int count)
  {
    if (m_lightsSpotCutoffUniform) {
      setUniformArray(m_lightsSpotCutoffUniform->location, value, count);
    }
  }

  void setLightsAttenuationUniform(const glm::vec3* value, int count)
  {
    if (m_lightsAttenuationUniform) {
      setUniformArray(m_lightsAttenuationUniform->location, value, count);
    }
  }

  void setLightsSpotExponentUniform(const float* value, int count)
  {
    if (m_lightsSpotExponentUniform) {
      setUniformArray(m_lightsSpotExponentUniform->location, value, count);
    }
  }

  void setLightsSpotDirectionUniform(const glm::vec3* value, int count)
  {
    if (m_lightsSpotDirectionUniform) {
      setUniformArray(m_lightsSpotDirectionUniform->location, value, count);
    }
  }

  void setMaterialSpecularUniform(const glm::vec4& value)
  {
    if (m_materialSpecularUniform) {
      setUniform(m_materialSpecularUniform->location, value);
    }
  }

  void setMaterialShininessUniform(float value)
  {
    if (m_materialShininessUniform) {
      setUniform(m_materialShininessUniform->location, value);
    }
  }

  void setMaterialAmbientUniform(const glm::vec4& value)
  {
    if (m_materialAmbientUniform) {
      setUniform(m_materialAmbientUniform->location, value);
    }
  }
  ...
  {
    if (m_orthoUniform) {
      setUniform(m_orthoUniform->location, value);
    }
  }

  void setSceneAmbientUniform(const glm::vec4& value)
  {
    if (m_sceneAmbientUniform) {
      setUniform(m_sceneAmbientUniform->location, value);
    }
  }
  ...
  {
    if (m_alphaUniform) {
      setUniform(m_alphaUniform->location, value);
    }
  }

  void setFogColorTopUniform(const glm::vec3& value)
  {
    if (m_fogColorTopUniform) {
      setUniform(m_fogColorTopUniform->location, value);
    }
  }

  void setFogColorBottomUniform(const glm::vec3& value)
  {
    if (m_fogColorBottomUniform) {
      setUniform(m_fogColorBottomUniform->location, value);
    }
  }
  ...
  {
    if (m_fogEndUniform) {
      setUniform(m_fogEndUniform->location, value);
    }
  }

  void setFogScaleUniform(float value)
  {
    if (m_fogScaleUniform) {
      setUniform(m_fogScaleUniform->location, value);
    }
  }

  void setFogDensityLog2eUniform(float value)
  {
    if (m_fogDensityLog2eUniform) {
      setUniform(m_fogDensityLog2eUniform->location, value);
    }
  }

  void setFogDensityDensityLog2eUniform(float value)
  {
    if (m_fogDensityDensityLog2eUniform) {
      setUniform(m_fogDensityDensityLog2eUniform->location, value);
    }
  }

  void setClipPlanesUniform(const glm::vec4* value, int count)
  {
    if (m_clipPlanesUniform) {
      setUniformArray(m_clipPlanesUniform->location, value, count);
    }
  }

  void setLightingEnabledUniform(bool value)
  {
    if (m_lightingEnabledUniform) {
      setUniform(m_lightingEnabledUniform->location, value);
    }
  }
  ...
  {
    if (m_color1Uniform) {
      setUniform(m_color1Uniform->location, value);
    }
  }
  ...
  {
    if (m_color2Uniform) {
      setUniform(m_color2Uniform->location, value);
    }
  }

  void setLineWidthUniform(float value)
  {
    if (m_lineWidthUniform) {
      setUniform(m_lineWidthUniform->location, value);
    }
  }

  void setBoxCorrectionUniform(float value)
  {
    if (m_boxCorrectionUniform) {
      setUniform(m_boxCorrectionUniform->location, value);
    }
  }

  void setCustomColorUniform(const glm::vec4& value)
  {
    if (m_customColorUniform) {
      setUniform(m_customColorUniform->location, value);
    }
  }

  void setUseCustomColorUniform(bool value)
  {
    if (m_useCustomColorUniform) {
      setUniform(m_useCustomColorUniform->location, value);
    }
  }
  ...
  {
    if (m_regionUniform) {
      setUniform(m_regionUniform->location, value);
    }
  }
