...
ROIGraphicsItem::ROIGraphicsItem(ZROI& roi,
                                 int slice,
                                 size_t id,
                                 ZView& view,
  ...
  , m_regionNode(regionNode)
{
  ...
  if (m_regionNode) {
    QString tooltip =
      QString("Region: %1 (%2 %3)").arg(m_regionNode->abbreviation).arg(m_regionNode->id).arg(m_regionNode->name);
    setToolTip(tooltip);
  }
  // todo: uncomment this when we have undo
  // setCursor(Qt::OpenHandCursor);
...
ROICtrlPtGraphicsItem::ROICtrlPtGraphicsItem(ZROI& roi,
...
//{
//   const auto& shapeOp = m_roi.shapeOperations(m_controlPoint.slice,
//   m_controlPoint.shapeID)[m_controlPoint.shapeIndex]; if (shapeOp.type == ROIType::Polygon || shapeOp.type ==
//   ROIType::Spline) {
//     QMenu menu;
  ...
  , m_regionNode(regionNode)
{
  ...
  connect(&m_highlightRegionOnMouseHover,
          ...
          this,
          &ZROIFilter::highlightRegionOnMouseHoverChanged);
...
void ZROIFilter::setData(ZROI& roi, ZRegionAnnotationPack& raPack)
{
  ...
  connect(m_regionAnnotationPack, &ZRegionAnnotationPack::lockedStateChanged, this, &ZROIFilter::onLockedStateChanged);
}
    ...
    if (!controlPoints.empty()) {
      if (modifiers == Qt::AltModifier) {
        ...
        for (auto& cp : controlPoints) {
          ...
          auto itemList =
            m_view.scene().items(QRectF(newScenePos - QPointF(15., 15.), newScenePos + QPointF(15., 15.)));
          double minDist = 15.;
          for (auto item : itemList) {
            if (item->isSelected() || !item->isVisible() ||
                item->type() != static_cast<int>(std::to_underlying(GraphicsItemType::ROICtrlPtGraphicsItem))) {
              continue;
            }
...
void ZROIFilter::createShapeItem(int slice, size_t shapeID)
{
  ...
  QPen pen(QColor(m_outlineColor.get().x * 255, m_outlineColor.get().y * 255, m_outlineColor.get().z * 255), 1);
...
void ZROIFilter::createCtrlPtItems(int slice, size_t shapeID)
{
  ...
  for (const auto& controlPoint : controlPoints) {
    auto rectItem = new ROICtrlPtGraphicsItem(*m_ROI,
                                              controlPoint,
                                              trans,
                                              m_view,
