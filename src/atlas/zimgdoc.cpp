...
void ZImgDoc::autoTrace()
{
  ...
  auto* watcher = new QFutureWatcher<AutoTraceAsyncResult>(&m_doc);
  connect(watcher,
          &QFutureWatcher<AutoTraceAsyncResult>::finished,
          &m_doc,
          [this, watcher, task, imgId, sc, outputSwcPath, outputLogPath, loadResult]() {
            CHECK(watcher != nullptr);
            AutoTraceAsyncResult r = watcher->result();
            watcher->deleteLater();

            ZBackgroundTaskManager& tm = m_doc.backgroundTaskManager();

            if (r.cancelled) {
              tm.cancelTask(task, QStringLiteral("cancelled"));
              return;
            }

            if (!r.error.isEmpty()) {
              tm.failTask(task, r.error);
              return;
            }

            if (!r.hasResult) {
              tm.succeedTask(task, QStringLiteral("no trace result"));
              return;
            }

            if (!loadResult) {
              tm.succeedTask(task, QStringLiteral("wrote %1").arg(QFileInfo(outputSwcPath).fileName()));
              return;
            }
            ...
            tm.succeedTask(task, QStringLiteral("created SWC #%1").arg(static_cast<qulonglong>(newSwcId)));
            ...
            m_doc.traceSettings().promoteNewSwcTargetToExistingIfStillNew(imgId, sc, newSwcId);
          });

  const QString traceCfgPath = QDir(ZSystemInfo::jsonDirPath()).absoluteFilePath("trace_config.json");

  QFuture<AutoTraceAsyncResult> future = QtConcurrent::run([imgPack,
                                                            sc,
                                                            t,
                                                            traceLevel,
                                                            traceCfgPath,
  ...
  task->setWaitForFinishedCallback([future]() mutable {
