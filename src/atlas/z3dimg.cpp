#include "z3dimg.h"
#include <algorithm>
#include <QString>
#include "QsLog.h"
#include "z3dshaderprogram.h"
#include "z3dgpuinfo.h"
#include "z3dtexture.h"

namespace nim {

Z3DImg::Z3DImg(nim::ZImg &img, const glm::vec3 &spacing,
               const glm::vec3 &offset, const glm::mat4 &transformation, QObject *parent)
  : QObject(parent)
  , m_histogramMaxValue(-1)
  , m_volColor(1.f,1.f,1.f)
{
  m_img.swap(img);
  m_dimensions = glm::uvec3(m_img.width(), m_img.height(), m_img.depth());
  m_detailVolumeDimensions = glm::round(glm::vec3(m_dimensions) * spacing);
  m_parentVolumeDimensions = m_detailVolumeDimensions;
  m_parentVolumeOffset = offset;
  setSpacing(spacing);
  setOffset(offset);
  setPhysicalToWorldMatrix(transformation);
  m_img.computeMinMax(m_minValue, m_maxValue);
}

Z3DImg::~Z3DImg()
{
  if (m_histogramThread) {
    if (m_histogramThread->isRunning()) {
      m_histogramThread->wait();
    }
  }
}

int Z3DImg::bitsStored() const
{
  if (m_img.isType<uint8_t>()) {
    return m_img.validBitCount() ? m_img.validBitCount() : 8;
  } else if (m_img.isType<uint16_t>()) {
    return m_img.validBitCount() ? m_img.validBitCount() : 16;
  } else if (m_img.isType<float>())
    return 32;
  return 0;
}

size_t Z3DImg::numVoxels() const
{
  return m_img.channelVoxelNumber();
}

QString Z3DImg::samplerType() const
{
  if (m_dimensions.z > 1)
    return "sampler3D";
  else if (m_dimensions.y > 1 && m_dimensions.x > 1)
    return "sampler2D";
  else
    return "sampler1D";
}

double Z3DImg::floatMinValue() const
{
  if (bitsStored() <= 16)
    return m_minValue / ((1 << bitsStored()) - 1);
  else
    return minValue();   // already float image
}

double Z3DImg::floatMaxValue() const
{
  if (bitsStored() <= 16)
    return m_maxValue / ((1 << bitsStored()) - 1);
  else
    return maxValue();   // already float image
}

double Z3DImg::value(int x, int y, int z) const
{
  return m_img.value<double>(x,y,z);
}

double Z3DImg::value(size_t index) const
{
  return m_img.value<double>(index);
}

void Z3DImg::asyncGenerateHistogram()
{
  if (hasHistogram() || m_histogramThread)
    return;
  m_histogramThread.reset(new Z3DImgHistogramThread(this));
  connect(m_histogramThread.get(), SIGNAL(finished()), this, SLOT(setHistogram()));
  m_histogramThread->start();
}

size_t Z3DImg::histogramBinCount() const
{
  if (m_img.isType<uint8_t>()) {
    return 256;
  } else {
    return m_histogram.size();
  }
  return 0;
}

size_t Z3DImg::histogramValue(size_t index) const
{
  if (index < m_histogram.size())
    return m_histogram[index];
  else
    return 0;
}

size_t Z3DImg::histogramValue(double fraction) const
{
  size_t index = static_cast<size_t>(fraction * static_cast<double>(histogramBinCount()-1));
  return histogramValue(index);
}

double Z3DImg::normalizedHistogramValue(size_t index) const
{
  return static_cast<double>(histogramValue(index)) / m_histogramMaxValue;
}

double Z3DImg::normalizedHistogramValue(double fraction) const
{
  size_t index = static_cast<size_t>(fraction * static_cast<double>(histogramBinCount()-1));
  return normalizedHistogramValue(index);
}

double Z3DImg::logNormalizedHistogramValue(size_t index) const
{
  return std::log(static_cast<double>(histogramValue(index))+1.0) / std::log(m_histogramMaxValue+1.0);
}

double Z3DImg::logNormalizedHistogramValue(double fraction) const
{
  size_t index = static_cast<size_t>(fraction * static_cast<double>(histogramBinCount()-1));
  return logNormalizedHistogramValue(index);
}

Z3DTexture *Z3DImg::texture()
{
  if (!m_texture) {
    generateTexture();
  }
  return m_texture.get();
}

void Z3DImg::setUniform(Z3DShaderProgram &shader, const QString &uniform, const GLint texUnit) const
{
  shader.setUniform(uniform + ".volume", texUnit);

  shader.setLogUniformLocationError(false);

  shader.setUniform(uniform + ".color", m_volColor);
  // volume size
  glm::vec3 dims = glm::vec3(dimensions());
  shader.setUniform(uniform + ".dimensions", dims);
  shader.setUniform(uniform + ".dimensions_RCP", glm::vec3(1.f) / dims);

  CHECK_GL_ERROR;

  shader.setLogUniformLocationError(true);
}

std::vector<double> Z3DImg::physicalBoundBox() const
{
  glm::vec3 luf = physicalLUF();
  glm::vec3 rdb = physicalRDB();
  std::vector<double> res(6);
  res[0] = luf.x;
  res[1] = rdb.x;
  res[2] = luf.y;
  res[3] = rdb.y;
  res[4] = luf.z;
  res[5] = rdb.z;
  return res;
}

std::vector<double> Z3DImg::worldBoundBox() const
{
  if (m_hasTransformMatrix) {
    std::vector<double> res(6);
    glm::vec3 minCoord = glm::min(worldLUF(), worldRDB());
    glm::vec3 maxCoord = glm::max(worldLUF(), worldRDB());

    minCoord = glm::min(minCoord, worldLDB());
    maxCoord = glm::max(maxCoord, worldLDB());

    minCoord = glm::min(minCoord, worldLDF());
    maxCoord = glm::max(maxCoord, worldLDF());

    minCoord = glm::min(minCoord, worldLUB());
    maxCoord = glm::max(maxCoord, worldLUB());

    minCoord = glm::min(minCoord, worldRUF());
    maxCoord = glm::max(maxCoord, worldRUF());

    minCoord = glm::min(minCoord, worldRDF());
    maxCoord = glm::max(maxCoord, worldRDF());

    minCoord = glm::min(minCoord, worldRUB());
    maxCoord = glm::max(maxCoord, worldRUB());

    res[0] = minCoord.x;
    res[1] = maxCoord.x;
    res[2] = minCoord.y;
    res[3] = maxCoord.y;
    res[4] = minCoord.z;
    res[5] = maxCoord.z;

    return res;
  } else {
    return physicalBoundBox();
  }
}

void Z3DImg::setPhysicalToWorldMatrix(const glm::mat4 &transformationMatrix)
{
  m_transformationMatrix = transformationMatrix;
  if (m_transformationMatrix != glm::mat4(1.0))
    m_hasTransformMatrix = true;
}

glm::mat4 Z3DImg::voxelToWorldMatrix() const
{
  return physicalToWorldMatrix() * voxelToPhysicalMatrix();
}

glm::mat4 Z3DImg::worldToVoxelMatrix() const
{
  return glm::inverse(voxelToWorldMatrix());
}

glm::mat4 Z3DImg::worldToTextureMatrix() const
{
  return glm::inverse(textureToWorldMatrix());
}

glm::mat4 Z3DImg::textureToWorldMatrix() const
{
  return voxelToWorldMatrix() * textureToVoxelMatrix();
}

glm::mat4 Z3DImg::voxelToPhysicalMatrix() const
{
  // 1. Multiply by spacing 2. Apply offset
  glm::mat4 scale = glm::scale(glm::mat4(1.0), spacing());
  return glm::translate(scale, offset());
}

glm::mat4 Z3DImg::physicalToVoxelMatrix() const
{
  glm::mat4 translate = glm::translate(glm::mat4(1.0), -offset());
  return glm::scale(translate, 1.f/spacing());
}

glm::mat4 Z3DImg::worldToPhysicalMatrix() const
{
  return glm::inverse(physicalToWorldMatrix());
}

glm::mat4 Z3DImg::textureToPhysicalMatrix() const
{
  return voxelToPhysicalMatrix() * textureToVoxelMatrix();
}

glm::mat4 Z3DImg::physicalToTextureMatrix() const
{
  return voxelToTextureMatrix() * physicalToVoxelMatrix();
}

glm::mat4 Z3DImg::textureToVoxelMatrix() const
{
  return glm::scale(glm::mat4(1.0), glm::vec3(dimensions()));
}

glm::mat4 Z3DImg::voxelToTextureMatrix() const
{
  return glm::scale(glm::mat4(1.0), 1.0f/glm::vec3(dimensions()));
}

void Z3DImg::setHistogram()
{
  m_histogram.swap(m_histogramThread->histogram());
  computeHistogramMaxValue();
  emit histogramFinished();
}

void Z3DImg::generateTexture()
{
  if (dimensions().x == 0 || dimensions().y == 0 || dimensions().z == 0) {
    QString message = QString("OpenGL volumes must have a size greater than 0 in all dimensions. Actual size: (%1, %2, %3)")
        .arg(m_img.width()).arg(m_img.height()).arg(m_img.depth());
    LERROR() << message;
    return;
  }

  GLenum format;
  GLint internalFormat;
  GLenum dataType;
  if (m_img.isType<uint8_t>()) {
    format = GL_RED;
    internalFormat = (GLint)GL_R8;
    dataType = GL_UNSIGNED_BYTE;
  } else if (m_img.isType<uint16_t>()) {
    format = GL_RED;
    internalFormat = (GLint)GL_R16;
    dataType = GL_UNSIGNED_SHORT;
  } else if (m_img.isType<float>()) {
    format = GL_RED;
    internalFormat = (GLint)GL_R32F;
    dataType = GL_FLOAT;
  } else {
    LERROR() << "Only GREY, GREY16 or FLOAT32 stack formats are supported";
    return;
  }

  // Create texture
  Z3DTexture* tex = new Z3DTexture(glm::ivec3(dimensions()), format, internalFormat, dataType);

  CHECK_GL_ERROR;
  tex->setData(m_img.channelData(0));
  tex->uploadTexture();

  CHECK_GL_ERROR;

  // append to internal data structure
  m_texture.reset(tex);
}

void Z3DImg::computeHistogramMaxValue()
{
  m_histogramMaxValue = *std::max_element(m_histogram.begin(), m_histogram.end());
}

//-----------------------------------------------------------------------------------
Z3DImgHistogramThread::Z3DImgHistogramThread(Z3DImg *volume, QObject* parent)
  : QThread(parent)
  , m_volume(volume)
{
}

void Z3DImgHistogramThread::run()
{
  m_histogram.assign(static_cast<size_t>(m_volume->maxValue()+1), 0);
  for (size_t i=0; i<m_volume->numVoxels(); i++) {
    m_histogram[static_cast<size_t>(m_volume->value(i))]++;
  }
}

void Z3DImg::translate(double dx, double dy, double dz)
{
  m_transformationMatrix[3][0] += dx;
  m_transformationMatrix[3][1] += dy;
  m_transformationMatrix[3][2] += dz;

  if (m_transformationMatrix != glm::mat4(1.0))
    m_hasTransformMatrix = true;
}

} // namespace nim


