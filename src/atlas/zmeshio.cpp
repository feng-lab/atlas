...
aiMesh* createAIMesh(const ZMesh& mesh)
{
  ...
  memcpy(pMesh->mVertices, &vertices[0], sizeof(float) * 3 * vertices.size());
  memcpy(pMesh->mNormals, &normals[0], sizeof(float) * 3 * vertices.size());
...
void ZMeshIO::load(const QString &filename, ZMesh &mesh) const
{
  ...
  if (filename.endsWith(".msh", Qt::CaseInsensitive)) {
    ...
    memcpy(&mesh.m_vertices[0], msh->mVertices, sizeof(float) * 3 * mesh.m_vertices.size());
    memcpy(&mesh.m_normals[0], msh->mNormals, sizeof(float) * 3 * mesh.m_vertices.size());
