...
bool Z3DRenderTarget::resize(const glm::uvec2& newsize)
{
  ...
  if (newsize.x > Z3DGpuInfo::instance().maxTextureSize() || newsize.y > Z3DGpuInfo::instance().maxTextureSize()) {
    LOG(WARNING) << "size " << newsize << " exceeds texture size limit: " << Z3DGpuInfo::instance().maxTextureSize();
    return false;
  }
...
bool Z3DRenderTarget::isFBOComplete()
{
  ...
  switch (status) {
    ...
    case GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER:
      LOG(ERROR)
        << "GL_FRAMEBUFFER_INCOMPLETE_READ_BUFFER: GL_READ_BUFFER is not GL_NONE "
    ...
    case GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE:
      LOG(ERROR)
        << "GL_FRAMEBUFFER_INCOMPLETE_MULTISAMPLE: the value of GL_RENDERBUFFER_SAMPLES is not the same "
...
void Z3DRenderTarget::saveAsColorImage(const QString& filename)
{
  ...
  try {
    ...
    std::unique_ptr<uint8_t[]>colorBuffer(new uint8_t[Z3DTexture::bypePerPixel(dataFormat, dataType) * m_size.x * m_size.y]);
...
void Z3DRenderTarget::saveAsDepthImage(const QString& filename)
{
  ...
  try {
    ...
    std::vector<uint32_t, boost::alignment::aligned_allocator<uint32_t, 64>> depthBuffer(m_size.x * m_size.y);
