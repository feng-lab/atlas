...
size_t ZRegionAnnotationDoc::loadFile(const json::value& jValue, QString& errorMsg)
{
  try {
...
json::value ZRegionAnnotationDoc::jsonValue(size_t id) const
{
  return json::value_from(m_idToRegionAnnotationPacks.at(id)->path());
}

bool ZRegionAnnotationDoc::isSameObj(const json::value& v1, const json::value& v2) const
{
...
void ZRegionAnnotationDoc::exportLabelImage()
{
  ...
  if (fmtIdx >= 0 && !fn.isEmpty()) {
    try {
      ...
      currentRegionAnnotationPack().regionAnnotation().exportLabelImage(fn, formats[fmtIdx], paras);
      ZSystemInfo::instance().addFileToRecentFileList(fn);
      ZSystemInfo::instance().setLastOpenedImagePath(fn);
    }
    catch (const ZException& e) {
...
size_t ZRegionAnnotationDoc::addRegionAnnotation(ZRegionAnnotation* regionAnnotation, const QString& path)
{
  ...
  connect(m_idToRegionAnnotationPacks[id].get(),
          &ZRegionAnnotationPack::undoStackCleanChanged,
          this,
          &ZRegionAnnotationDoc::setModified);
  return id;
}
