  ...
  if (m_image != NULL) {
    ...
    if (m_tileimages.size() == m_pTiles->size()) {
      ...
      for (int i=0; i<m_pTiles->size(); i++) {
        ...
        painter.drawImage(QRectF(tl.x(), tl.y(), br.x()-tl.x()+m_scaleFactor, br.y()-tl.y()+m_scaleFactor), m_tileimages[i],
        ...
        if (m_pTiles->at(i).bIsSelected) {
          ...
          painter.drawRect(QRectF(tl.x()-4, tl.y()-4, br.x()-tl.x()+m_scaleFactor+4, br.y()-tl.y()+m_scaleFactor+4));
...
QLayout *ZStitchImageDialog::createIOLayout()
{
  ...
  pl->setToolTip(tr("Use the interval to downsample stack while stitching (more memory efficient). Note that this interval will be applied to original stack, not downsampled one."));
  layout->addWidget(pl, row, 0);
...
void ZStitchImageDialog::stitchStacks2()
{
  ...
  if (m_inputStack1Filenames.size() == 1 && m_useTileImageRadioButton->isChecked() && m_tileList.size() > 1) {
    ...
    filepath2 = NULL;
    nstack = m_nSel;
  } else if ((m_inputStack1Filenames.size() == 1 && m_inputStack2Filenames.size() == 1) || m_useLayoutRadioButton->isChecked()) {
      ...
      if (m_nSel >= 0) {
        ...
        if (m_inputStack1Filenames.size() != m_nSel && m_inputStack1Filenames.size() != m_tileList.size()) {
          m_commandOutputEdit->append(QString("<font color=red>The number of input stacks 1 (%1) is not equal to either \
                                              ...
                                              Abort.</font>").arg(m_inputStack1Filenames.size()).arg(m_nSel).arg(m_tileList.size()));
                                              return;
        }
        nstack = m_nSel;
      ...
      if (m_nSel >= 0) {
        ...
        if (m_inputStack2Filenames.size() != m_nSel && m_inputStack2Filenames.size() != m_tileList.size()) {
          m_commandOutputEdit->append(QString("<font color=red>The number of input stacks 2 (%1) is not equal to either \
                                              number of selected tiles (%2) or number of all tiles (%3). \
                                              Can not decide which files should be stitiched.  \
                                              Abort.</font>").arg(m_inputStack2Filenames.size()).arg(m_nSel).arg(m_tileList.size()));
                                              return;
        }
  ...
  if (m_useLayoutRadioButton->isChecked()) {
    ...
    if (nstack != 1) {  // check if the number fit
      if (nstack != tmp_nstack) {
        m_commandOutputEdit->append(QString("<font color=red>Invalid layout: number of input stacks: %1, required input stacks: %2. Abort.</font>").arg(nstack).arg(tmp_nstack));
        return;
      }
      ...
      for (int j=0; j<tileMatrix[0].size(); j++) {
        if (tileMatrix[i][j] > 0) {
          ...
          if (i+1 < tileMatrix.size() && j-1 >= 0 && tileMatrix[i+1][j-1] > 0
              && tileMatrix[i][j-1] == 0 && tileMatrix[i+1][j] == 0) {  // down-left, add only if left and down are empty
            int idx1 = tileMatrix[i][j]-1;
            int idx2 = tileMatrix[i+1][j-1]-1;
            conn[idx1][idx2] = 1;
            conn[idx2][idx1] = 1;
          ...
          if (connected == false) {
            m_commandOutputEdit->append(QString("<font color=red>Can not stitch because images are not connected. Abort.</font>"));
            QApplication::processEvents();
  ...
  for(int i=nstack;i<2*nstack-1;i++) {
    for(int j=i+1;j<2*nstack;j++) {
      if ((conn == NULL) || (conn[i-nstack][j-nstack] == 1)) {
        max_corr[idx] = Align_Stack_MR_D(downstacks[i],downstacks[j],intv,
...
//  }
  ...
  for (i = 0; i < 3; i++) {
    corner1[i] = iarray_min(start[i], nstack*2, NULL);
  }
  for (i = 0; i < 3; i++) {
    corner2[i] = iarray_max(end[i], nstack*2, NULL);
  }
  ...
  if (m_useTileImageRadioButton->isChecked() && m_tileImage != NULL) {
  ...
  // cleanup
  if (m_useLayoutRadioButton->isChecked() && m_inputStack1Filenames.size() == 1 && m_layout1SpinBox->value() * m_layout2SpinBox->value() != 1) {
    for (int i=0; i<nstack; i++) {
      free(filepath1[i]);
      free(filepath2[i]);
    }
...
QString ZStitchImageDialog::save(const QString &filepath, const Mc_Stack *mc_stack)
{
  if (mc_stack->nchannel == 1 || (mc_stack->nchannel == 3 && mc_stack->kind == 1)) {  //should be fine
    Write_Mc_Stack(filepath.toLocal8Bit().constData(), mc_stack, NULL);
    return filepath;
  } else {  //save as raw
    QString str = filepath;
    if (!str.endsWith(".raw", Qt::CaseInsensitive)) {
      m_commandOutputEdit->append("<font color=red>Current stack can not be saved as tif, use raw format instead.</font>");
      str += ".raw";
    }
    Write_Mc_Stack(str.toLocal8Bit().constData(), mc_stack, NULL);
    return str;
  }
}
...
//  }
  ...
  if (nstack == 1) {
    if (m_dsCheckBox->isChecked()) {
      ...
      if ((Is_Tiff(input1ba.data())) && Is_Tiff(outputba.data())) {
        ...
        Mc_Stack *stack = Read_Mc_Stack(input1ba.data(), -1);
        //Write_Mc_Stack(outputba.data(), stack, NULL);
        save(m_outputFileEdit->text(), stack);
        Kill_Mc_Stack(stack);
      }
    }
  ...
  if (m_concatOnlyCheckBox->isChecked()) {
    ...
    if (conn == NULL) {
      m_commandOutputEdit->append("<font color=red>Position information incomplete for image concatenate, Abort.</font>");
      for (int i=0; i<nstack; i++) {
        free(all_config[i]);
      }
      free(all_config);
    ...
    for (int i = 1; i < nstack; i++) {
      ...
      if (!std::equal(stackSizes[0], stackSizes[0]+3, stackSizes[i])) {
        m_commandOutputEdit->append("<font color=red>For image concatenate, all the stack size must be same. Abort.</font>");
...
char** ZStitchImageDialog::splitstack(Mc_Stack *stack, const char *filepath, int nstack)
{
  ...
  if (!fileDir.cd("tmp")) {  //create tmp folder
    if (!fileDir.mkdir("tmp")) {
      m_commandOutputEdit->append(QString("<font color=red>Can not create folder: %1. Abort.</font>").arg(fileDir.absolutePath()+"/tmp"));
      return NULL;
    }
  } else {
