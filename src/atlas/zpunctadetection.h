...
public:
  ...
  void setResultPunctaFilename(const QString &fn) { m_detectedPunctaFileName = fn; }
  void setResultSomaPunctaFilename(const QString &fn) { m_detectedSomaPunctaFileName = fn; }

  // if not set, auto threshold will be used
  void setPunctaThreshold(int thre) { m_punctaThreshold = thre; }
  // miminum voxel number to goto split step, default is 20 voxels
  void setSplitThreshold(int thre) { m_splitSizeThreshold = thre; }
  // confidence region of gaussian, used to estimate punctum radius, default is 0.95
  void setConfidenceRegionForRadiusEstimate(double c) { m_confRadius = c; }
  // confidence region of gaussian, used to get overlap area of error ellipse, default is 0.8
  void setConfidenceRegionForOverlapArea(double c) { m_confOverlapArea = c; }
  // overlap threshold to merge two gaussian, default is 0.8
  void setOverlapRateThreshold(double t) { m_overlapRateThreshold = t; }
  // for watershed
  void setSeedSizeThreshold(int i) { m_seedSizeThreshold = i; }
  // default is true
  ...
  void setDendriteChannel(int c) { m_dendriteChannel = c; }
  // for soma detection
  // default use 2.6um
  void setMaxDendriteTubeRadiusInUm(double mtr) { m_maxDendriteTubeRadius = mtr; }
  void setDendriteThreshold(double tt) { m_dendriteThreshold = tt; }

  // assign puncta to each swc tree if set
  void clearAllSwcTrees() { m_swcTrees.clear(); }
  ...
  void setMaxDistToBranchInUm(double d) { m_maxDistToBranch = d; }
  ...
  void setAmbiguousFactor(double f) { m_ambiguousFactor = f; }
