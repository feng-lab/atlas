...
public:
  ...
  void listenTo(const QString& actionName,
  ...
  {
    if (!m_listeningToContextMenuEvent) {
      m_listeningToContextMenuEvent = true;
      Q_EMIT valueChanged();
    }
  }
...
protected:
  struct MouseEvent
  {
    MouseEvent(QString actionName_, Qt::MouseButtons buttons_, Qt::KeyboardModifiers modifiers_, QEvent::Type type_)
      : actionName(std::move(actionName_))
      , buttons(buttons_)
  ...
  struct KeyEvent
  {
    KeyEvent(QString actionName_, Qt::Key key_, Qt::KeyboardModifiers modifiers_, QEvent::Type type_ = QEvent::KeyPress)
      : actionName(std::move(actionName_))
      , key(key_)
      , modifiers(modifiers_)
      , type(type_)
