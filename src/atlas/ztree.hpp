...
protected:
  ValueType& dereference() const { return node->data; }

  bool isTail(const NodeType *node) { return !node->parent && !node->nextSibling; }
};
...
protected:
  ...
  void decrement()
  {
    assert(startNode && this->node != startNode);
    ...
    typename TBaseIter::ValueType&>
{
  struct enabler {};
public:
  Iterator()  { this->init(nullptr, nullptr); }
  template <class OtherTBaseIter>
  Iterator(Iterator<OtherTBaseIter> const& other, typename std::enable_if<
           std::is_convertible<typename OtherTBaseIter::ValueType*, typename TBaseIter::ValueType*>::value
           , enabler
           >::type = enabler())
  { this->init(other.node, other.parent); }
  explicit Iterator(typename TBaseIter::NodeType* n, typename TBaseIter::NodeType* p = nullptr) { this->init(n, p); }

  bool operator<(const Iterator<TBaseIter>& rhs) const { return this->node < rhs.node; }

protected:
  friend class boost::iterator_core_access;

  bool equal(Iterator rhs) const { return this->node == rhs.node; }
};
...
public:
  ...
  virtual ~ZTree() { clear(); delete m_head; delete m_tail; }

  ZTree<T>& operator=(ZTree rhs) { swap(rhs); return *this; }
  void swap(ZTree<T> &rhs) noexcept { std::swap(m_head, rhs.m_head); std::swap(m_tail, rhs.m_tail); }

  Iterator begin() { return Iterator(m_head->nextSibling); }
  Iterator end() { return Iterator(m_tail); }
  template<typename Iter>
  Iterator begin(const Iter& root)
  {
    assert(root.node && root.node != m_head && root.node != m_tail);
  ...
  ConstIterator begin() const { return ConstIterator(m_head->nextSibling); }
  ...
  template<typename Iter>
  ConstIterator cbegin(const Iter& root) const { return begin(root); }
  template<typename Iter>
  ConstIterator cend(const Iter& root) const { return end(root); }
  ...
  PostOrderIterator endPost() { return PostOrderIterator(m_tail); }
  template<typename Iter>
  ...
  ConstPostOrderIterator endPost() const { return ConstPostOrderIterator(m_tail); }
  template<typename Iter>
  ...
  ConstPostOrderIterator cbeginPost() const { return beginPost(); }
  ConstPostOrderIterator cendPost() const { return endPost(); }
  template<typename Iter>
  ConstPostOrderIterator cbeginPost(const Iter& root) const { return beginPost(root); }
  template<typename Iter>
  ConstPostOrderIterator cendPost(const Iter& root) const { return endPost(root); }
  BreadthFirstIterator beginBreadthFirst() { return BreadthFirstIterator(m_head->nextSibling); }
  BreadthFirstIterator endBreadthFirst() { return BreadthFirstIterator(); }
  template<typename Iter>
  ...
  ConstBreadthFirstIterator beginBreadthFirst() const { return ConstBreadthFirstIterator(m_head->nextSibling); }
  ConstBreadthFirstIterator endBreadthFirst() const { return ConstBreadthFirstIterator(); }
  template<typename Iter>
  ...
  ConstBreadthFirstIterator cbeginBreadthFirst() const { return beginBreadthFirst(); }
  ConstBreadthFirstIterator cendBreadthFirst() const { return endBreadthFirst(); }
  template<typename Iter>
  ConstBreadthFirstIterator cbeginBreadthFirst(const Iter& root) const { return beginBreadthFirst(root); }
  template<typename Iter>
  ConstBreadthFirstIterator cendBreadthFirst(const Iter& root) const { return endBreadthFirst(root); }
  RootIterator beginRoot() { return RootIterator(m_head->nextSibling); }
  RootIterator endRoot() { return RootIterator(m_tail); }
  ConstRootIterator beginRoot() const { return ConstRootIterator(m_head->nextSibling); }
  ConstRootIterator endRoot() const { return ConstRootIterator(m_tail); }
  ConstRootIterator cbeginRoot() const { return beginRoot(); }
  ConstRootIterator cendRoot() const { return endRoot(); }
  template<typename Iter>
  ChildIterator beginChild(const Iter& parent)
  {
    assert(parent.node && parent.node != m_head && parent.node != m_tail);
  ...
  template<typename Iter>
  ConstChildIterator cbeginChild(const Iter& parent) const { return beginChild(parent); }
  template<typename Iter>
  ConstChildIterator cendChild(const Iter& parent) const { return endChild(parent); }
  template<typename Iter>
  AncestorIterator beginAncestor(const Iter& child, bool includeChild = false)
  {
    assert(child.node && child.node != m_head && child.node != m_tail);
  ...
  {
    assert(child.node && child.node != m_head && child.node != m_tail);
    return includeChild ? ConstAncestorIterator(child.node, child.node) : ConstAncestorIterator(child.node->parent, child.node);
  }
  template<typename Iter>
  ...
  template<typename Iter>
  ConstAncestorIterator cbeginAncestor(const Iter& child, bool includeChild = false) const { return beginAncestor(child, includeChild); }
  template<typename Iter>
  ConstAncestorIterator cendAncestor(const Iter& child) const { return endAncestor(child); }
  ...
  LeafIterator endLeaf() { return LeafIterator(m_tail); }
  template<typename Iter>
  ...
  ConstLeafIterator endLeaf() const { return ConstLeafIterator(m_tail); }
  template<typename Iter>
  ...
  ConstLeafIterator cbeginLeaf() const { return beginLeaf(); }
  ConstLeafIterator cendLeaf() const { return endLeaf(); }
  template<typename Iter>
  ConstLeafIterator cbeginLeaf(const Iter& root) const { return beginLeaf(root); }
  template<typename Iter>
  ConstLeafIterator cendLeaf(const Iter& root) const { return endLeaf(root); }
  ...
  template<typename Iter>
  size_t numAncestors(const Iter& child) const { return std::distance(beginAncestor(child), endAncestor(child)); }
  template<typename Iter>
  size_t numDescendants(const Iter& parent) const { return size(parent) - 1; }
  bool empty() const { return m_head->nextSibling == m_tail; }
  template<typename Iter>
  static bool isRoot(const Iter& pos) { return !pos.node->parent; }
  template<typename Iter>
  static bool isBranchNode(const Iter& pos) { return pos.node->firstChild != pos.node->lastChild; }
  template<typename Iter>
  static bool isLeaf(const Iter& pos) { return !pos.node->firstChild; }
  template<typename Iter>
  ...
  void appendChild(Iter parent, Iter child)
  {
    ...
    if (parent == this->parent(child))
      return;
    detachParent(child);
    assert(!child.node->parent && !child.node->prevSibling && !child.node->nextSibling);
  ...
  {
    assert(to.node && to.node != m_head && to.node != m_tail && from.node);
    eraseChildren(to);
  ...
  {
    assert(pos.node && pos.node != m_head && pos.node != m_tail);
...
protected:
  ...
  void reverseChildRoot(Iter child, Iter root)
  {
    assert(child.node && child.node != m_head && child.node != m_tail);
    assert(root.node && root.node != m_head && root.node != m_tail);
    assert(root == this->parent(child) && isRoot(root));
    detachParent(child);
