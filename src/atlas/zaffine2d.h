...
public:
  ...
  void setTranslation(double x, double y) { m_translationX = x; m_translationY = y; }
  void setRotationAngle(double radianAngle) { m_rotateAngle = radianAngle; }
  void setScale(double xscale, double yscale) { m_scaleX = xscale; m_scaleY = yscale; }
  void setShear(double xy, double yx) { m_shearXY = xy; m_shearYX = yx; }
  void makeMatrix();

  double translationX() const { return m_translationX; }
  double translationY() const { return m_translationY; }
  double rotationAngle() const { return m_rotateAngle; }
  ...
  // access
  const Eigen::Matrix3d& transformMatrix() const { return m_matrix; }
  const Eigen::Matrix3d& inverseTransformMatrix() const { return m_inverseMatrix; }
