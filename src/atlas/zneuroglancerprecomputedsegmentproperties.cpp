  ...
  std::chrono::milliseconds timeout)
{
  ...
  if (inlineIt == root.end() || inlineIt->value().is_null()) {
    // No inline properties; treat as empty.
    return props;
  }
  if (!inlineIt->value().is_object()) {
    throw ZException("Invalid segment properties info: 'inline' must be an object");
  }
  const auto& inl = inlineIt->value().as_object();

  // ids
  auto idsIt = inl.find("ids");
  if (idsIt == inl.end() || !idsIt->value().is_array()) {
    throw ZException("Invalid segment properties info: missing or invalid inline.ids (expected array)");
  }
  const auto& idsArr = idsIt->value().as_array();
  props->m_ids.reserve(idsArr.size());
  for (const auto& v : idsArr) {
    if (!v.is_string()) {
      throw ZException("Invalid segment properties info: inline.ids must contain strings");
    }
  ...
  if (propsIt == inl.end() || !propsIt->value().is_array()) {
    throw ZException("Invalid segment properties info: missing or invalid inline.properties (expected array)");
  }
  ...
  for (const auto& pv : propsArr) {
    if (!pv.is_object()) {
      throw ZException("Invalid segment properties info: inline.properties elements must be objects");
    }
