...
void ZColorMap::create1DTexture(size_t width)
{
  ...
  if (maxTexSize < width)
    width = maxTexSize;
  m_texture.reset(new Z3DTexture(GL_TEXTURE_1D, (GLint)GL_RGBA8, glm::uvec3(width,1,1), GL_BGRA, GL_UNSIGNED_INT_8_8_8_8_REV));
  CHECK_GL_ERROR;
}
...
void ZColorMapParameter::readValue(const QJsonValue &jsonValue)
{
