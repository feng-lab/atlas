...
class Z3DImgHistogramThread : public QThread
{
public:
  Z3DImgHistogramThread(Z3DImg *volume, QObject* parent = 0);
  void run();
  std::vector<size_t>& histogram() { return m_histogram; }
...
public:
  ...
  glm::uvec3 originalDimensions() const { return m_detailVolumeDimensions; }
  // parent stack dimension, detailed stack might be cropped form parent stack
  glm::uvec3 parentVolumeDimensions() const { return m_parentVolumeDimensions; }
  ...
  glm::vec3 spacing() const { return m_spacing; }
  ...
  glm::vec3 physicalLUF() const { return offset(); }
  glm::vec3 physicalRDB() const { return offset() + cubeSize() - spacing(); }
  ...
  glm::vec3 worldLUF() const { return m_hasTransformMatrix ? glm::applyMatrix(m_transformationMatrix, physicalLUF()) : physicalLUF(); }
  glm::vec3 worldRDB() const { return m_hasTransformMatrix ? glm::applyMatrix(m_transformationMatrix, physicalRDB()) : physicalRDB(); }
  glm::vec3 worldLDF() const { return m_hasTransformMatrix ? glm::applyMatrix(m_transformationMatrix, physicalLDF()) : physicalLDF(); }
  glm::vec3 worldRDF() const { return m_hasTransformMatrix ? glm::applyMatrix(m_transformationMatrix, physicalRDF()) : physicalRDF(); }
  glm::vec3 worldRUF() const { return m_hasTransformMatrix ? glm::applyMatrix(m_transformationMatrix, physicalRUF()) : physicalRUF(); }
  glm::vec3 worldLUB() const { return m_hasTransformMatrix ? glm::applyMatrix(m_transformationMatrix, physicalLUB()) : physicalLUB(); }
  glm::vec3 worldLDB() const { return m_hasTransformMatrix ? glm::applyMatrix(m_transformationMatrix, physicalLDB()) : physicalLDB(); }
  glm::vec3 worldRUB() const { return m_hasTransformMatrix ? glm::applyMatrix(m_transformationMatrix, physicalRUB()) : physicalRUB(); }

  // xmin, xmax, ymin, ymax, zmin, zmax
  std::vector<double> worldBoundBox() const;

  // corners of parent volume
  glm::vec3 parentVolPhysicalLUF() const { return m_parentVolumeOffset; }
  glm::vec3 parentVolPhysicalRDB() const { return m_parentVolumeOffset + (glm::vec3(m_parentVolumeDimensions)-1.f); }
  glm::vec3 parentVolPhysicalLDF() const { return glm::vec3(parentVolPhysicalLUF().x, parentVolPhysicalRDB().y, parentVolPhysicalLUF().z); }
  glm::vec3 parentVolPhysicalRDF() const { return glm::vec3(parentVolPhysicalRDB().x, parentVolPhysicalRDB().y, parentVolPhysicalLUF().z); }
  glm::vec3 parentVolPhysicalRUF() const { return glm::vec3(parentVolPhysicalRDB().x, parentVolPhysicalLUF().y, parentVolPhysicalLUF().z); }
  glm::vec3 parentVolPhysicalLUB() const { return glm::vec3(parentVolPhysicalLUF().x, parentVolPhysicalLUF().y, parentVolPhysicalRDB().z); }
  glm::vec3 parentVolPhysicalLDB() const { return glm::vec3(parentVolPhysicalLUF().x, parentVolPhysicalRDB().y, parentVolPhysicalRDB().z); }
  glm::vec3 parentVolPhysicalRUB() const { return glm::vec3(parentVolPhysicalRDB().x, parentVolPhysicalLUF().y, parentVolPhysicalRDB().z); }

  glm::vec3 parentVolWorldLUF() const
  {
    return !isSubVolume() ? worldLUF() :
                            m_hasTransformMatrix ? glm::applyMatrix(m_transformationMatrix, parentVolPhysicalLUF()) : parentVolPhysicalLUF();
  }
  glm::vec3 parentVolWorldRDB() const
  {
    return !isSubVolume() ? worldRDB() :
                            m_hasTransformMatrix ? glm::applyMatrix(m_transformationMatrix, parentVolPhysicalRDB()) : parentVolPhysicalRDB();
  }
  glm::vec3 parentVolWorldLDF() const
  {
    return !isSubVolume() ? worldLDF() :
                            m_hasTransformMatrix ? glm::applyMatrix(m_transformationMatrix, parentVolPhysicalLDF()) : parentVolPhysicalLDF();
  }
  glm::vec3 parentVolWorldRDF() const
  {
    return !isSubVolume() ? worldRDF() :
                            m_hasTransformMatrix ? glm::applyMatrix(m_transformationMatrix, parentVolPhysicalRDF()) : parentVolPhysicalRDF();
  }
  glm::vec3 parentVolWorldRUF() const
  {
    return !isSubVolume() ? worldRUF() :
                            m_hasTransformMatrix ? glm::applyMatrix(m_transformationMatrix, parentVolPhysicalRUF()) : parentVolPhysicalRUF();
  }
  glm::vec3 parentVolWorldLUB() const
  {
    return !isSubVolume() ? worldLUB() :
                            m_hasTransformMatrix ? glm::applyMatrix(m_transformationMatrix, parentVolPhysicalLUB()) : parentVolPhysicalLUB();
  }
  glm::vec3 parentVolWorldLDB() const
  {
    return !isSubVolume() ? worldLDB() :
                            m_hasTransformMatrix ? glm::applyMatrix(m_transformationMatrix, parentVolPhysicalLDB()) : parentVolPhysicalLDB();
  }
  glm::vec3 parentVolWorldRUB() const
  {
    return !isSubVolume() ? worldRUB() :
                            m_hasTransformMatrix ? glm::applyMatrix(m_transformationMatrix, parentVolPhysicalRUB()) : parentVolPhysicalRUB();
  }

  void setSpacing(const glm::vec3 &spacing) { m_spacing = spacing; }
  ...
  glm::vec3 volColor() const { return m_volColor; }
  ...
  glm::mat4 physicalToWorldMatrix() const { return m_transformationMatrix; }
