...
public:
  inline size_t numImages() const { return m_qimages.size(); }
  inline QImage& image(size_t n) { return m_qimages[n]; }
  inline QPoint& location(size_t n) { return m_locations[n]; }
  inline const QImage& image(size_t n) const { return m_qimages[n]; }
  inline const QPoint& location(size_t n) const { return m_locations[n]; }
  inline double scale(size_t n) const { return m_scales[n]; }

  inline void addImage(const QImage &image, const QPoint &loc, double scale = 1.0)
    { m_qimages.push_back(image); m_locations.push_back(loc); m_scales.push_back(scale); }
};
