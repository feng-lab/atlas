...
public:
  ...
  inline Z3DCameraParameter& globalCameraPara() { return m_rendererBase.globalCameraPara(); }
  ...
  const std::vector<double>& notTransformedBoundBox() const { return m_notTransformedBoundBox; }

  // Useful coordinate L->Left U->Up F->Front R->Right D->Down B->Back
  glm::vec3 physicalLUF() const { return glm::vec3(m_notTransformedBoundBox[0], m_notTransformedBoundBox[2], m_notTransformedBoundBox[4]); }
  glm::vec3 physicalRDB() const { return glm::vec3(m_notTransformedBoundBox[1], m_notTransformedBoundBox[3], m_notTransformedBoundBox[5]); }
  ...
  glm::vec3 worldLUF() const { return glm::applyMatrix(m_rendererBase.coordTransform(), physicalLUF()); }
  glm::vec3 worldRDB() const { return glm::applyMatrix(m_rendererBase.coordTransform(), physicalRDB()); }
  glm::vec3 worldLDF() const { return glm::applyMatrix(m_rendererBase.coordTransform(), physicalLDF()); }
  glm::vec3 worldRDF() const { return glm::applyMatrix(m_rendererBase.coordTransform(), physicalRDF()); }
  glm::vec3 worldRUF() const { return glm::applyMatrix(m_rendererBase.coordTransform(), physicalRUF()); }
  glm::vec3 worldLUB() const { return glm::applyMatrix(m_rendererBase.coordTransform(), physicalLUB()); }
  glm::vec3 worldLDB() const { return glm::applyMatrix(m_rendererBase.coordTransform(), physicalLDB()); }
  glm::vec3 worldRUB() const { return glm::applyMatrix(m_rendererBase.coordTransform(), physicalRUB()); }

  virtual bool hasOpaque(Z3DEye) const { return m_rendererBase.opacity() == 1.f; }
  virtual void renderOpaque(Z3DEye) {}
  virtual bool hasTransparent(Z3DEye) const { return m_rendererBase.opacity() < 1.f; }
