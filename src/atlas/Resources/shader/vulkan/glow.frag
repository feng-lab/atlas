...
layout(push_constant) uniform GlowPC {
  ...
  uint color_texture;
  uint depth_texture;
  uint glowmap_color_texture;
  uint glowmap_depth_texture;
} pc;
...
void main()
{
  ...
  vec4 dst = texture(atlas_bindlessSampler2DLinear(pc.color_texture), tc);
  vec4 src = texture(atlas_bindlessSampler2DLinear(pc.glowmap_color_texture), tc);
  if (src.a == 0.0 && dst.a == 0.0) discard;

  if (GLOW_MODE == 0) { // ADDITIVE_BLENDING
    FragData0 = min(src + dst, 1.0);
    float depth = texture(atlas_bindlessSampler2DLinear(pc.depth_texture), tc).r;
    gl_FragDepth = depth < 1.0 ? depth : texture(atlas_bindlessSampler2DLinear(pc.glowmap_depth_texture), tc).r;
    ...
    FragData0 = src;
    gl_FragDepth = texture(atlas_bindlessSampler2DLinear(pc.glowmap_depth_texture), tc).r;
  }
}
