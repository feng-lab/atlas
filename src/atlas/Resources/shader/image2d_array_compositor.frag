uniform sampler2DArray color_texture;
uniform sampler2DArray depth_texture;

#if GLSL_VERSION >= 330
layout(location = 0) out vec4 FragData0;
#elif GLSL_VERSION >= 130
out vec4 FragData0;  // call glBindFragDataLocation before linking
#else
#define FragData0 gl_FragData[0]
#endif

void main()
{
#if NUM_VOLUMES < 2
  FragData0 = texelFetch(color_texture, ivec3(gl_FragCoord.xy, 0), 0);;
  gl_FragDepth = texelFetch(depth_texture, ivec3(gl_FragCoord.xy, 0), 0).r;;
#elif defined(MIP)
  vec4 color = texelFetch(color_texture, ivec3(gl_FragCoord.xy, 0), 0);
  float depth = texelFetch(depth_texture, ivec3(gl_FragCoord.xy, 0), 0).r;
  for (int i = 1; i < NUM_VOLUMES; ++i) {
    color = max(color, texelFetch(color_texture, ivec3(gl_FragCoord.xy, i), 0));
    depth = min(depth, texelFetch(depth_texture, ivec3(gl_FragCoord.xy, i), 0).r);
  }
  FragData0 = color;
  gl_FragDepth = depth;
#else
  vec4 colors[NUM_VOLUMES];
  float depths[NUM_VOLUMES];
  vec4 keyColor;
  float keyDepth;
  for (int i = 0; i < NUM_VOLUMES; ++i) {
    colors[i] = texelFetch(color_texture, ivec3(gl_FragCoord.xy, i), 0);
    depths[i] = texelFetch(depth_texture, ivec3(gl_FragCoord.xy, i), 0).r;
  }

  for (int j = 1; j < NUM_VOLUMES; ++j) {
    keyColor = colors[j];
    keyDepth = depths[j];
    int i = j-1;
    while (i >= 0 && keyDepth > depths[i]) {
      depths[i+1] = depths[i];
      colors[i+1] = colors[i];
      --i;
    }
    depths[i+1] = keyDepth;
    colors[i+1] = keyColor;
  }

  vec4 color = colors[1] + (1 - colors[1].a) * colors[0];
  for (int i = 2; i < NUM_VOLUMES; ++i) {
    color = colors[i] + (1 - colors[i].a) * color;
  }
  FragData0 = color;
  gl_FragDepth = depths[NUM_VOLUMES-1];
#endif
}
