...
public:
  ...
  void setBoundInBorder(bool v) { m_boundInBorder = v; }
#endif

  void setPadOption(PadOption po) { m_padOption = po; }
  void setInterpolant(Interpolant ip) { m_interpolant = ip; }
  PadOption padOption() const { return m_padOption; }
  void setFillValue(double v) { m_fillValue = v; }
  Interpolant interpolant() const { return m_interpolant; }
...
protected:
  ...
  inline bool inBound(size_t width, size_t height, size_t depth, double x, double y, double z) const
    { return x >= 0. && x < static_cast<double>(width) && y >= 0. && y < static_cast<double>(height) && z >= 0. && z < static_cast<double>(depth); }
