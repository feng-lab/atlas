                                        ...
                                        vk::raii::CommandBuffer& cmd)
{
  ...
  if (m_backend.ddpIndirectCountEnabled() && shaderHook == Z3DRendererBase::ShaderHookType::DualDepthPeelingInit) {
    ...
    CHECK(payload.streamKey != 0) << "Cone DDP init: missing streamKey";
    DDPArgs& ddp = m_ddpArgsByStream[DDPStreamKey{payload.streamKey, payload.streamSegmentOrdinal}];
    prepareDdpArgs(ddp, indexed, "Cone DDP init");
    recorder.recordGraphicsDraw(drawSpec);
  } else if (m_backend.ddpIndirectCountEnabled() &&
             shaderHook == Z3DRendererBase::ShaderHookType::DualDepthPeelingPeel) {
    const bool indexed = (drawSpec.indexCount > 0);
    CHECK(payload.streamKey != 0) << "Cone DDP peel: missing streamKey";
    DDPArgs& ddp = m_ddpArgsByStream[DDPStreamKey{payload.streamKey, payload.streamSegmentOrdinal}];
    if (!ddp.prepared) {
      prepareDdpArgs(ddp, indexed, "Cone DDP peel");
      recorder.recordGraphicsDraw(drawSpec);
      return;
    }
    CHECK(ddp.indexed == indexed) << "Cone DDP peel: indexed mode mismatch for streamKey";
    CHECK(ddp.indexCount == drawSpec.indexCount) << "Cone DDP peel: index count mismatch";
    CHECK(ddp.vertexCount == drawSpec.vertexCount) << "Cone DDP peel: vertex count mismatch";
    recorder.recordGraphicsDraw(drawSpec, [&](vk::raii::CommandBuffer& c) {
      ...
      CHECK(argsBuf != vk::Buffer{}) << "Cone DDP peel: device args buffer missing";
      CHECK(cntBuf != vk::Buffer{}) << "Cone DDP peel: indirect count buffer missing";
      if (indexed) {
