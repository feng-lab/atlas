  ...
  try {
    ...
    if (hostSwcOpt.has_value()) {
      ...
      if (imgPack->isDiskCached()) {
        ...
        tr = traceSeedIntoHostSwcLegacyLike(signalVol, hostSwcOpt->second, seed, cfg, zToXYRatio, cancellationToken);
      } else {
        ...
        tr =
          traceSeedIntoHostSwcLegacyLike(signal, hostSwcOpt->second, seed, cfg, zToXYRatio, sc, t, cancellationToken);
      }
      if (tr.swc) {
      ...
      if (imgPack->isDiskCached()) {
        const ZImgPackVoxelVolume signalVol(imgPack, sc, t);
        tr = traceSeedNewSwcLegacyLike(signalVol, seed, cfg, zToXYRatio, cancellationToken);
      } else {
        const ZImg& signal = imgPack->img();
        if (signal.isEmpty()) {
          res.error = QStringLiteral("Trace failed: the image is empty.");
        ...
        tr = traceSeedNewSwcLegacyLike(signal, seed, cfg, zToXYRatio, sc, t, cancellationToken);
      }
      if (tr.swc) {
        ZNeutubeSwcResampler resampler;
                               ...
                               std::function<void(size_t newSwcId)> onNewSwcCreated)
{
  ...
  const double zToXYRatio =
    traceSettings.zToXYRatioOverrideForSelection(sourceImgObjId, sc).value_or(derivedZToXYRatio);
