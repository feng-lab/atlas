...
public:
  ...
  inline void setSlice(size_t z) { m_z = std::min(z, m_imgPack.imgInfo().depth-1); }
  inline void setTime(size_t t) { m_t = std::min(t, m_imgPack.imgInfo().numTimes-1); }
  inline void setScale(double s) { m_scale = s; }
  inline void setViewport(const QRectF &rect) { m_viewport = rect; }
