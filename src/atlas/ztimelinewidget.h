...
public:
  ...
  ZAnimation& animation() { return m_animation; }
  inline int rowHeight() const { return 30; }
  inline double pixelsPerSecond() const { return m_pixelsPerSecond; }

  inline int objViewWidth() const { return 220; }
  inline int minViewHeight() const { return 10; }
  inline int eventViewWidth() const { return m_eventViewWidth; }
  inline double timeToX(double time) const { return 10 + time * pixelsPerSecond(); }
  inline double xToTime(double x) const { return (x - 10) / pixelsPerSecond(); }
