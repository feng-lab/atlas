...
template<bool ReportProgress>
template<typename TVoxel>
TVoxel ZImgAutoThreshold<ReportProgress>::typedCentroidThre(double &cent1, double &cent2, const ZImg& imgIn, size_t c, size_t t)
{
  if (!imgIn.isType<TVoxel>()) {
    throw ZImgException("input img voxel type doesnot match provided type");
  }
  ...
  do {
    ...
    size_t totalWeight = std::accumulate(hist.begin()+low, hist.begin()+threBin+1, size_t(0));
    if (totalWeight == 0) {
    ...
    totalWeight = std::accumulate(hist.begin()+threBin+1, hist.begin()+high+1, size_t(0));
    if (totalWeight == 0) {
