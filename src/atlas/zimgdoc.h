...
public:
  ...
  virtual QString typePluralName() const override { return "Images"; }
...
protected:
  ...
  size_t loadImg(const QString &fileName, size_t scene, FileFormat format, QString &errorMsg, size_t numScene = 0, const ZImgInfo *info = nullptr,
  ...
  size_t loadImg(const QStringList &files, Dimension catDim, size_t scene, FileFormat format, QString &errorMsg, size_t numScene = 0, const ZImgInfo *info = nullptr,
                 const std::vector<std::shared_ptr<ZImgSubBlock>> *subBlock = nullptr);
