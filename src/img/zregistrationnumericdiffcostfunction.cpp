                                                    ...
                                                    double* gradient) const
{
  ...
  imgTypeDispatcher(m_fixedImg->info(), [=, this]<typename TFixed>() {
    imgTypeDispatcher(m_movingImg->info(), [=, this]<typename TMoving>() {
  ...
  if (gradient) {
    ...
    for (size_t i = 0; i < paras.size(); ++i) {
      ...
      imgTypeDispatcher(m_fixedImg->info(), [&, this]<typename TFixed>() {
        imgTypeDispatcher(m_movingImg->info(), [&, this]<typename TMoving>() {
          this->evaluate_Impl<TFixed, TMoving>(paraPlusDelta.data(), &newValue);
        });
      });
...
void ZRegistrationNumericDiffCostFunction::evaluate_Impl(const double* const parameters, double* value) const
{
  ...
  ZImg movingBuffer(ZImgInfo(m_fixedImg->width(),
                             m_fixedImg->height(),
                             m_fixedImg->depth(),
                             1,
                             1,
                             m_movingImg->bytesPerVoxel(),
                             m_movingImg->voxelFormat()));
  if (m_transform->is2DTransform()) {
    m_transform->transformImage(m_movingImg->channelData<TMoving>(0),
                                m_movingImg->width(),
                                m_movingImg->height(),
                                movingBuffer.channelData<TMoving>(0),
                                0,
                                m_fixedImg->width(),
                                0,
                                m_fixedImg->height());
  ...
  *value = m_metric->value(m_fixedImg->channelData<TFixed>(0),
                           movingBuffer.channelData<TMoving>(0),
                           m_fixedImg->width(),
                           m_fixedImg->height(),
                           m_fixedImg->depth());
