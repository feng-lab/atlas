...
public:
  ...
  bool isNormalImage() const { return !isReducedResolutionImage() && !isTransparencyMask(); }

  bool isReducedResolutionImage() const;
  bool isMultipageDocument() const;
  bool isTransparencyMask() const;

  inline bool containsTag(uint64_t tag) const { return indexOf(tag) != -1; }
  const ZImgMetatag &tag(size_t idx) const { return m_entries[idx]; }
  ...
  uint64_t nextIFDOffset() const { return m_nextIFDOffset; }

  inline void addField(const ZImgMetatag &fd) { m_entries.push_back(fd); }
  inline void addSubIFD(const ZTiffIFD &sub) { m_subIFDs.push_back(sub); }
  inline void setOffset(uint64_t off) { m_offset = off; }
  inline void setNextIFDOffset(uint64_t off) { m_nextIFDOffset = off; }
  QString toQString() const;

  const std::vector<ZTiffIFD>& subIFDs() const { return m_subIFDs; }
...
public:
  ...
  inline void addIFD(const ZTiffIFD &ifd) { m_ifds.push_back(ifd); }
  QString toQString() const;

  const std::vector<ZTiffIFD> &ifds() const { return m_ifds; }

  // default is true, call before load and readInfo
  void setUseColormap(bool v) { m_useColormap = v; }
  bool isUsingColormap() const { return m_useColormap; }
  ...
  void close() { m_ifds.clear(); m_tif.reset(); }
  bool isValid() const { return !m_ifds.empty(); }
  bool isLsmFile() const;
  // crash if file is not lsm
  const ZImgMetatag& lsmInfoTag() const;

  // read
  inline void readInfoFromIFD(size_t ifdIdx, ZImgInfo &info) { readInfoFromIFD(m_ifds[ifdIdx], info); }
  ...
  static void writeTiffHeader(uint8_t *mem, size_t width, size_t height, size_t bitsPerSample, size_t samplesPerPixel, size_t compression,
...
protected:
  ...
  void copyTileToImg(const uint8_t* tileBuf, size_t tileWidth, size_t tileHeight, size_t numChannels, size_t voxelByteNumber,
...
public:
  ...
  void finishWriting() { m_tif.reset(); }
