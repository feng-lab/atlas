...
class img_neighborhood_iter
    ...
    img_neighborhood_iter<TImg, TVoxel>
    , TVoxel
...
public:
  ...
  img_neighborhood_iter(img_neighborhood_iter<OtherTImg, OtherTVoxel> const& other, typename std::enable_if<
  ...
  __forceinline const ZNeighborhood& neighborhood() const { return m_neighborhood; }
...
protected:
  ...
  template <typename, typename> friend class img_neighborhood_iter;

  template <typename OtherTImg, typename OtherTVoxel>
  __forceinline bool equal(img_neighborhood_iter<OtherTImg, OtherTVoxel> const& other) const
  {
  ...
  template <typename OtherTImg, typename OtherTVoxel>
  __forceinline int64_t distance_to(img_neighborhood_iter<OtherTImg, OtherTVoxel> const& other) const
  {
...
// similar to ZImgRegionIterator, provide access to neighborhood of current voxel :

// ZImgNeighborhoodIterator<uint8_t> it = ZImgNeighborhoodIterator<uint8_t>(img, region);
// it.setNeighborhood(6);
// for (; !it.isAtEnd(); ++it) {
//   *it = 0;   // set center voxel to 0
