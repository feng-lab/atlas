...
struct ZImgRegion
{
  ...
  ZImgRegion()
    ...
    , end(-1,-1,-1,-1,-1)
  {}
  ZImgRegion(value_type xStart, value_type xEnd,
             value_type yStart, value_type yEnd,
             value_type zStart = 0, value_type zEnd = -1,
             value_type cStart = 0, value_type cEnd = -1,
             value_type tStart = 0, value_type tEnd = -1)
  ...
  inline bool isValid(const ZImgInfo &info) const
  {
    return start.allGreaterEqual(0) &&
        start.allLessThan(ZVoxelCoordinate(info.width, info.height, info.depth,
  ...
  {
    return start.x == 0 && start.y == 0 && start.z == 0 && start.c == 0 && start.t == 0 &&
        end.x == -1 && end.y == -1 && end.z == -1 && end.c == -1 && end.t == -1;
  }

  // check if this is an empty region
  inline bool isEmpty() const
  {
    return start.anyLessThan(0) ||
        (end.x != -1 && start.x >= end.x) ||
        (end.y != -1 && start.y >= end.y) ||
        (end.z != -1 && start.z >= end.z) ||
        (end.c != -1 && start.c >= end.c) ||
        (end.t != -1 && start.t >= end.t);
  }

  bool containsCoord(const ZVoxelCoordinate& coord, const ZImgInfo &info) const;
  ...
  ZImgInfo clip(const ZImgInfo &info) const;

  inline bool xInRegion(value_type x) const { return x >= start.x && (end.x == -1 || x < end.x); }
  inline bool yInRegion(value_type y) const { return y >= start.y && (end.y == -1 || y < end.y); }
  inline bool zInRegion(value_type z) const { return z >= start.z && (end.z == -1 || z < end.z); }
  inline bool cInRegion(value_type c) const { return c >= start.c && (end.c == -1 || c < end.c); }
  inline bool tInRegion(value_type t) const { return t >= start.t && (end.t == -1 || t < end.t); }
  //inline bool lInRegion(value_type l) const { return l >= start.l && (end.l == -1 || l < end.l); }

  inline value_type xStart() const { return start.x; }
  inline value_type xEnd() const { return end.x; }
  inline value_type yStart() const { return start.y; }
  inline value_type yEnd() const { return end.y; }
  inline value_type zStart() const { return start.z; }
  inline value_type zEnd() const { return end.z; }
  inline value_type cStart() const { return start.c; }
  inline value_type cEnd() const { return end.c; }
  inline value_type tStart() const { return start.t; }
  inline value_type tEnd() const { return end.t; }
