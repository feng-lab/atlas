...
struct CZ_LsmInfo
{
  ...
  int32_t s32DimensionZ; // Number of intensity values in z-direction or in case of scan mode "Time Series Mean-of-ROIs"
                         // the Number of ROIs.
  ...
  // 0 in case of different data types for different channels. In the latter case the field 32OffsetChannelDataTypes
  ...
  // 0 - no spectral scan.
  // 1 - image has been acquired in spectral scan mode with a LSM 510 META or LSM 710 QUASAR detector (release 3.0 or
  // later).
  ...
  uint32_t u32OffsetInputLut; // File offset to the channel input LUT with brightness and contrast properties (can be 0,
                              // if not present).
  uint32_t u32OffsetOutputLut; // File offset to the color palette (can be 0, if not present).
  uint32_t
    u32OffsetChannelColors; // File offset to the list of channel colors and channel names (can be 0, if not present).
  double f64TimeInterval; // Time interval for time series in "s" (can be 0, if it is not a time series or if there is
                          // more detailed information in u32OffsetTimeStamps).
  uint32_t u32OffsetChannelDataTypes; // File offset to an array with uint32_t-values with the format of the intensity
                                      // values for the respective channels (can be 0, if not present).
  // The contents of the array elements are
  //   1 - for 8-bit unsigned integer,
  //   2 - for 12-bit unsigned integer and
  //   5 - for 32-bit float (for "Time Series Mean-of-ROIs" ).
  uint32_t u32OffsetScanInformation; // File offset to a structure with information of the device settings used to scan
                                     // the image (can be 0, if not present).
  uint32_t u32OffsetKsData; // File offset to "Zeiss Vision KS-3D" specific data (can be 0, if not present).
  uint32_t u32OffsetTimeStamps; // File offset to a structure containing the time stamps for the time indexes (can be 0,
                                // if it is not a time series).
  uint32_t u32OffsetEventList; // File offset to a structure containing the experimental notations recorded during a
                               // time series (can be 0, if not present).
  uint32_t u32OffsetRoi; // File offset to a structure containing a list of the ROIs used during the scan operation (can
                         // be 0, if not present).
  uint32_t u32OffsetBleachRoi; // File offset to a structure containing a description of the bleach region used during
                               // the scan operation (can be 0, if not present).
  uint32_t u32OffsetNextRecording; // For "Time Series Mean-of-ROIs" and for "Line scans" it is possible that a second
                                   // image is stored in the file (can be 0, if not present).
  // For "Time Series Mean-of-ROIs" it is an image with the ROIs. For "Line scans" it is the image with the selected
  // line. In these cases u32OffsetNextRecording contains a file offset to a second file header. This TIFF-header and
  // all sub-structures are built exactly the same way as a simple LSM 5/7 file. All offsets without exception are given
  // there relative to the start of the second TIFF-header.
  ...
  uint32_t u32OffsetMeanOfRoisOverlay; // File offset to the description of the vector overlay with the ROIs used during
                                       // a scan in "Mean of ROIs" mode (can be 0, if not present).
  uint32_t
    u32OffsetTopoIsolineOverlay; // File offset to the description of the vector overlay for the topography-iso-lines
                                 // and height display with the profile selection line (can be 0, if not present).
  uint32_t u32OffsetTopoProfileOverlay; // File offset to the description of the vector overlay for the
                                        // topography-profile display (can be 0, if not present).
  uint32_t u32OffsetLinescanOverlay; // File offset to the description of the vector overlay for the line scan line
                                     // selection with the selected line or Bezier curve (can be 0, if not present).
  uint32_t u32ToolbarFlags; // Bit-field for disabled toolbar buttons:
  // bit 0 - "Corp" button
  // bit 1 - "Reuse" button.
  // If the bit is set the corresponding button is disabled.
  uint32_t u32OffsetChannelWavelength; // Offset to memory block with the wavelength range used during acquisition for
                                       // the individual channels (new for release 3.0; can be 0, if not present).
  uint32_t u32OffsetChannelFactors; // Offset to memory block with scaling factor, offset and unit for each image
                                    // channel. The data are currently used by images with ion concentration data.
  ...
  double f64ObjectiveSphereCorrection; // The inverse radius of the spherical error of the objective that was used
                                       // during acquisition.
  // This is the radius of the sphere that can be fitted on the topography reconstruction of an absolutely plane object
  // that has been recorded with the objective.
  uint32_t u32OffsetUnmixParameters; // File offset to the parameters for linear unmixing that have been used to
                                     // generate the image data from scan data of the spectral detector
  //(new for release 3.2; can be 0, if not present).
  uint32_t
    u32OffsetAcquisitionParameters; // File offset to a block with acquisition parameters for support of the re-use
                                    // function of the LSM 5/7 program (new for release 3.5; can be 0, if not present).
  uint32_t u32OffsetCharacteristics; // File offset to a block with user specified properties (new for release 3.5; can
                                     // be 0, if not present).
  uint32_t u32OffsetPalette; // File offset to a block with detailed color palette properties (new for release 3.5; can
                             // be 0, if not present).
  double f64TimeDifferenceX; // The time difference for the acquisition of adjacent pixels in x-direction in seconds.
                             // The property is used by RICS analysis (new for release 5.0; can be 0, if not present).
  double f64TimeDifferenceY; // The time difference for the acquisition of adjacent pixels in y-direction in seconds.
                             // The property is used by RICS analysis (new for release 5.0; can be 0, if not present).
  double f64TimeDifferenceZ; // The time difference for the acquisition of adjacent pixels in z-direction in seconds.
                             // The property is used by RICS analysis (new for release 5.0; can be 0, if not present).
...
struct CZ_ChannelColors
{
  ...
  int32_t
    s32NumberNames; // Number of character strings for the channel names; should be the same as the number of channels.
