...
struct ZImgInfo
{
  ...
  inline static size_t numDimensions() { return 5; }

  // access
  inline void setSize(Dimension dim, size_t size) { (&width)[enumToUnderlyingType(dim)] = size; if (dim >= Dimension::C) createDefaultDescriptions(); }
  // sz should has at least numDimensions() elements
  inline void setSize(size_t* sz) { memcpy(&width, sz, sizeof(size_t)*6); createDefaultDescriptions(); }
  inline size_t size(Dimension dim) const { return (&width)[enumToUnderlyingType(dim)]; }
  inline size_t size(size_t dim) const { return (&width)[dim]; }
  // note: time stride is meaningless since the memory is not contiguous
  inline size_t stride(Dimension dim) const { size_t res = 1; for (int i=0; i<enumToUnderlyingType(dim); ++i) res *= (&width)[i]; return res; }
  inline size_t stride(size_t dim) const { size_t res = 1; for (size_t i=0; i<dim; ++i) res *= (&width)[i]; return res; }
  inline size_t& operator[](size_t i) { return (&width)[i]; }
  ...
  inline size_t rowByteNumber() const { return width * bytesPerVoxel; }
  inline size_t planeVoxelNumber() const { return width * height; }
  inline size_t planeByteNumber() const { return width * height * bytesPerVoxel; }
  inline size_t channelVoxelNumber() const { return width * height * depth; }
  inline size_t channelByteNumber() const { return width * height * depth * bytesPerVoxel; }
  inline size_t timeVoxelNumber() const { return width * height * depth * numChannels; }
  inline size_t timeByteNumber() const { return width * height * depth * numChannels * bytesPerVoxel; }
  ...
  inline size_t voxelNumber() const { return width * height * depth * numChannels * numTimes; }
  inline size_t byteNumber() const { return width * height * depth * numChannels * numTimes * bytesPerVoxel; }
};
#pragma pack(pop)
