...
std::vector<TFloat> create1DGaussianKernel(TFloat sigmaX, index_t width = -1, size_t* kWidth = nullptr);

// same as fspecial('gaussian', sz, sigma)
// if width == -1 then default size if sigma * 6 and it is returned in kWidth
template<typename TFloat>
std::vector<TFloat> create2DGaussianKernel(TFloat sigmaX,
                                           TFloat sigmaY,
                                           index_t width = -1,
                                           index_t height = -1,
                                           size_t* kWidth = nullptr,
                                           size_t* kHeight = nullptr);
...
//  if width == -1 then default size if sigma * 6 and it is returned in kWidth
template<typename TFloat>
std::vector<TFloat> create1DLoGKernel(TFloat sigmaX, index_t width = -1, size_t* kWidth = nullptr);
