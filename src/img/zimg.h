...
public:
  ...
  void load(const QStringList &fileList, Dimension catDim, size_t scene = 0, FileFormat format = FileFormat::Unknown, bool expandXY = false,
            bool expandWithMaxValue = false);
  void load(const QStringList &fileList, Dimension catDim, const ZImgRegion &region, size_t scene = 0, FileFormat format = FileFormat::Unknown, bool expandXY = false,
            bool expandWithMaxValue = false);
  void load(const ZImgSource &imgSource);
  void save(const QString &filename, FileFormat format = FileFormat::Unknown, Compression comp = Compression::AUTO) const;

  // convenient function to get img information from file, throw ZIOException if read error or empty image
  static std::vector<ZImgInfo> readImgInfo(const QString &filename, std::vector<std::vector<std::shared_ptr<ZImgSubBlock>>> *subBlocks = nullptr,
                                           FileFormat format = FileFormat::Unknown);
  // throw ZIOException if sequence is not valid or empty image
  static std::vector<ZImgInfo> readImgInfo(const QStringList &fileList, Dimension catDim, std::vector<std::vector<std::shared_ptr<ZImgSubBlock>>> *subBlocks = nullptr,
  ...
  inline bool isEmpty() const { return m_data.empty() || m_info.isEmpty(); }
  inline bool hasThumbnail() const { return !m_thumbnail.isEmpty(); }

  inline const ZImgThumbernail& thumbnail() const { return m_thumbnail; }
  inline const ZImgInfo& info() const { return m_info; }
  inline const ZImgMetadata& metadata() const { return m_metadata; }

  inline ZImgThumbernail& thumbnailRef() { return m_thumbnail; }
  inline ZImgInfo& infoRef() { return m_info; }
  inline ZImgMetadata& metadataRef() { return m_metadata; }

  template<typename TVoxel>
  bool isType() const;

  inline bool isSameType(const ZImg &other) const { return m_info.isSameType(other.m_info); }

  inline bool isSameSize(const ZImg &other) const { return m_info.isSameSize(other.m_info); }
  ...
  inline size_t size(Dimension dim) const { return m_info.size(dim); }
  inline size_t size(size_t dim) const { return m_info.size(dim); }
  //inline bool isMultiLocationsImg() const { return m_info.numLocations > 1; }
  inline bool isTimeSeries() const { return m_info.numTimes > 1; }
  inline bool isMultiChannelsImg() const { return m_info.numChannels > 1; }
  // imgs that have x,y,z dimensions and don't have t,l dimensions are 3D Img, x,y dimensions can be singleton
  inline bool is3DImg() const { return !isEmpty() && m_info.numTimes == 1 && m_info.depth > 1; }
  // imgs that have x,y dimensions and don't have z,t,l dimensions are 2D Img, x,y dimensions can be singleton
  inline bool is2DImg() const { return !isEmpty() && m_info.numTimes == 1 && m_info.depth == 1; }

  inline size_t voxelByteNumber() const { return m_info.voxelByteNumber(); }
  inline size_t rowVoxelNumber() const { return m_info.rowVoxelNumber(); }
  inline size_t rowByteNumber() const { return m_info.rowByteNumber(); }
  inline size_t planeVoxelNumber() const { return m_info.planeVoxelNumber(); }
  inline size_t planeByteNumber() const { return m_info.planeByteNumber(); }
  inline size_t channelVoxelNumber() const { return m_info.channelVoxelNumber(); }
  inline size_t channelByteNumber() const { return m_info.channelByteNumber(); }
  inline size_t timeVoxelNumber() const { return m_info.timeVoxelNumber(); }
  inline size_t timeByteNumber() const { return m_info.timeByteNumber(); }
  ...
  inline size_t voxelNumber() const { return m_info.voxelNumber(); }
  inline size_t byteNumber() const { return m_info.byteNumber(); }

  inline size_t width() const { return m_info.width; }
  inline size_t height() const { return m_info.height; }
  inline size_t depth() const { return m_info.depth; }
  inline size_t numChannels() const { return m_info.numChannels; }
  inline size_t numTimes() const { return m_info.numTimes; }
  //inline size_t numLocations() const { return m_info.numLocations; }
  inline VoxelFormat voxelFormat() const { return m_info.voxelFormat; }
  inline size_t bytesPerVoxel() const { return m_info.bytesPerVoxel; }
  inline size_t validBitCount() const { return m_info.validBitCount; }
  inline VoxelSizeUnit voxelSizeUnit() const { return m_info.voxelSizeUnit; }
  inline double voxelSizeX() const { return m_info.voxelSizeX; }
  inline double voxelSizeY() const { return m_info.voxelSizeY; }
  inline double voxelSizeZ() const { return m_info.voxelSizeZ; }
  // if current or result voxelSizeUnit is Voxel, result is meaningless
  inline double voxelSizeXInUnit(VoxelSizeUnit unit) const { return m_info.voxelSizeXInUnit(unit); }
  inline double voxelSizeYInUnit(VoxelSizeUnit unit) const { return m_info.voxelSizeYInUnit(unit); }
  inline double voxelSizeZInUnit(VoxelSizeUnit unit) const { return m_info.voxelSizeZInUnit(unit); }
  inline double voxelSizeXInUm() const { return voxelSizeXInUnit(VoxelSizeUnit::um); }
  inline double voxelSizeYInUm() const { return voxelSizeYInUnit(VoxelSizeUnit::um); }
  inline double voxelSizeZInUm() const { return voxelSizeZInUnit(VoxelSizeUnit::um); }
  ...
  { return reinterpret_cast<T*>(&(m_data[t][0]) + c * m_info.channelByteNumber() + z * m_info.planeByteNumber() + y * m_info.rowByteNumber()); }
  ...
  { return reinterpret_cast<T*>(&(m_data[coord.t][0]) + coord.c * m_info.channelByteNumber() + coord.z * m_info.planeByteNumber() +
  ...
  inline bool isImgView() const { return !m_ownData; }
  ...
  TValue dataRangeMin() const { return m_info.dataRangeMin<TValue>(); }
  template<typename TValue = double>
  TValue dataRangeMax() const { return m_info.dataRangeMax<TValue>(); }
  ...
  inline ZImg binarized() const { return binarized(0, ThresholdMode::ExcludeThreshold); }
  // return a uint8_t img with 1 and 0
  ...
  inline ZVoxelCoordinate indexToCoord(int64_t idx) const { return indexToCoord(idx, m_info); }
  inline int64_t coordToIndex(const ZVoxelCoordinate& coord) const { return coordToIndex(coord, m_info); }
  ...
  inline ZVoxelCoordinate maxCoord() const
  {
    ...
    return ZVoxelCoordinate(m_info.width-1, m_info.height-1, m_info.depth-1, m_info.numChannels-1, m_info.numTimes-1);
  }
  ...
  // padValue is only used when padOption is PadOption::Constant, padValue will be cast to img voxel type
  template <typename TValue = double>
  TValue valueWithPad(const ZVoxelCoordinate& coord, PadOption padOption = PadOption::Constant, TValue padValue = TValue(0)) const;
  // overload
  template <typename TValue = double>
...
private:
  ...
  template<typename TVoxel>
  void blockDownsampled_Impl(ZImg &res, size_t blockWidth, size_t blockHeight, size_t blockDepth, CombineMode mode) const;
