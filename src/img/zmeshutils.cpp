...
using vtkCCSPoly = std::vector<int64_t>;
...
using vtkCCSPolyGroup = std::vector<size_t>;

// Extra info for each edge in a poly
using vtkCCSPolyEdges = std::vector<int64_t>;
                   ...
                   size_t exhaustive)
{
  ...
  using vtkCCSCutLoc = std::pair<double, size_t>;
...
//    VLOG(1) << result.numTriangles();
//    for (size_t i = 0; i < result.numVertices(); ++i) {
//      VLOG(1) << result.vertices()[i] << " " << result.textureCoordinates3D()[i];
//    }
//    for (size_t i = 0; i < result.numTriangles(); ++i) {
//      VLOG(1) << result.triangleIndices()[i];
//    }
