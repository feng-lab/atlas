...
//-----------------------------------------------------------------------------
SIZE_T EnumerateUsingDXGI( IDXGIFactory* pDXGIFactory )
{
    ...
    for( UINT index = 0; ; ++index )
    {
        ...
        if( SUCCEEDED( pAdapter->GetDesc( &desc ) ) )
        {
            ...
            res = std::max(res, desc.DedicatedVideoMemory / 1024 / 1024);
        }
...
//-----------------------------------------------------------------------------
SIZE_T EnumerateUsingD3D9( IDirect3D9* pD3D9 )
{
    assert( pD3D9 != 0 );
    ...
    for( UINT iAdapter = 0; iAdapter < dwAdapterCount; iAdapter++ )
    {
        ...
        //                        dwAdapterRAM );
        //            res = std::max(res, (SIZE_T)dwAdapterRAM / 1024 / 1024);
        //        } else
        //            wprintf( L"\tGetVideoMemoryViaWMI\n\t\tn/a\n" );
        ...
        if( SUCCEEDED( GetVideoMemoryViaD3D9( hMonitor, &dwAvailableTextureMem ) ) ) {
            ...
            res = std::max(res, (SIZE_T)dwAvailableTextureMem / 1024 / 1024);
        } else
            wprintf( L"\tGetVideoMemoryViaD3D9\n\t\tn/a\n" );
    }
    return res;
...
uint64_t getDedicatedVideoMemoryMB()
{
  ...
  if( hDXGI )
  {
    ...
    if ( SUCCEEDED(hr) )
    {
      res = EnumerateUsingDXGI( pDXGIFactory );
