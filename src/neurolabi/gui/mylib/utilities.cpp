...
#undef  TEXT    //  windows.h declares this!
...
void *mylib::Guarded_Malloc(size_t size, const char *routine)
{ void *p;
...
void *mylib::Guarded_Realloc(void *p, size_t size, const char *routine)
{ p = realloc(p,size);
  if (p == NULL)
    { fprintf(stderr,"\nError in %s:\n",routine);
      fprintf(stderr,"   Out of memory\n");
...
char *mylib::Guarded_Strdup(char *p, const char *routine)
{ p = strdup(p);
  if (p == NULL)
    { fprintf(stderr,"\nError in %s:\n",routine);
      fprintf(stderr,"   Out of memory\n");
...
FILE *mylib::Guarded_Fopen(char *file_name, char *options, const char *routine)
{ FILE *f;
...
{ Node *p;
  ...
  p = (Node *) mylib::Guarded_Malloc(sizeof(Node),"Process_Arguments");
  p->label = label;
  p->left  = left;
  p->right = right;
  switch (p->label)
...
{ Atom *p;
  ...
  p = (Atom *) mylib::Guarded_Malloc(sizeof(Atom),"Process_Arguments");
  p->label   = label;
  p->empty   = (label == WHITE);
  p->loops   = 0;
  p->name    = name;
...
static const char *C2_ERROR = "Can call Process_Arguments only once!";

static const char *RP_ERROR = "Missing right paren, ), in command-line specification";
static const char *RC_ERROR = "Missing right curly brace, }, in command-line specification";
static const char *RB_ERROR = "Missing right bracket, ], in command-line specification";
static const char *RA_ERROR = "Missing right angle bracket, >, in command-line specification";
static const char *WP_ERROR = "White space inside argument angle brackets <>";
static const char *UT_ERROR = "Invalid type in command-line specification";
static const char *PT_ERROR = "Prematurely terminated command-line specification";
static const char *CC_ERROR = "Cannot have {} within another {}";
static const char *UO_ERROR = "Unterminated <> token";
static const char *UD_ERROR = "Unterminated default in ()'s";
static const char *UB_ERROR = "End of line immediately after escape symbol";
static const char *EI_ERROR = "Expecting integer default value";
static const char *ED_ERROR = "Expecting floating point default value";
static const char *EN_ERROR = "Cannot have ... within the scope of another ...";

static const char *TO_ERROR = "Type is not part of an option";
static const char *TT_ERROR = "Type must be separated from a preceding type by white space or text";
static const char *TL_ERROR = "Type cannot be part of an option flag list";
static const char *NR_ERROR = "Name should have white space or nothing to its right";
static const char *NL_ERROR = "Name should have white space or nothing to its left";
static const char *TX_ERROR = "Text follows a type chain";
static const char *OP_ERROR = "Start of option flag list should have white space or nothing to its left";
static const char *BO_ERROR = "Start of option should begin with a -";
static const char *CW_ERROR = "Cannot have white space within {}";
static const char *CT_ERROR = "Cannot have a type within {}";
static const char *CP_ERROR = "Curly braces can only enclose option text";
static const char *PL_ERROR = "Cannot have only part of a unit within the scope of a ...";
static const char *OL_ERROR = "Cannot have a pure option within the scope of a ...";

static const char *DP_ERROR = "Unit occurs twice on an expansion of the specification";
static const char *DF_ERROR = "Two instances of the same unit have different default values";
...
{ if (a->label == NAME)
    return ((int) ((a->name - Master_Spec[a->line]) - 1));
  else
    return ((int) (a->name - Master_Spec[a->line]));
}

static void semantic_error(const char *msg, Atom *s)
{ fprintf(stderr,"\nError in Process_Arguments:\n  %s\n",msg);
...
static void dual_error(const char *msg, Atom *a, Atom *b)
{ fprintf(stderr,"\nError in Process_Arguments:\n  %s\n",msg);
          ...
          { char *valptr, *e;
            ...
            *Scan = '\0';
            if (type_def.type == INT_TYPE)
              { type_def.value.intval = (int) strtol(valptr,&e,10);
                if (*valptr == '\0' || *e != '\0')
                  { Scan = valptr;
    ...
    default:
      if (isspace(*Scan))                              // White space
        ...
        { int   kind;
          ...
          p = new_atom(kind,string,(int) (Scan-string),NULL,Specline,0);
        }
      break;
  }
...
{ State *s;
  ...
  s = (State *) mylib::Guarded_Malloc(sizeof(State),"Process_Arguments");
            ...
            if (unit[i]->label == TYPE)
              ...
              { if (vi >= Stop) goto nomatch;
                if (unit[i]->nlen != AtStk[vi]->nlen)
                  goto nomatch;
                if (strncmp(unit[i]->name,AtStk[vi]->name,(size_t) (unit[i]->nlen)))
                  goto nomatch;
...
{ int   has_curly, k;
  ...
  if (Num_of_Units >= Unit_Table_Max)
    { Unit_Table_Max = (int) (1.2*Num_of_Units + 100);
      Unit_Table = (Unit *)
        mylib::Guarded_Realloc(Unit_Table,sizeof(Unit)*((size_t) Unit_Table_Max),"Process_Arguments");
    }
...
{ AtStk     = (Atom **) mylib::Guarded_Malloc(sizeof(Atom *)*((size_t) mach->longest),"Process_Arguments");
  Stop      = 0;
  Unit_Span = 0;
...
{ Candidate *mat = (Candidate  *) mylib::Guarded_Malloc(sizeof(Candidate),"Process_Argument");
  mat->next  = def->clist;
  mat->argp  = argp;
  def->clist = mat;
  return (mat);
...
{ int m;
  ...
  { case INT_TYPE:
      m = match_int(arg,(int) strlen(arg));
      break;
    case DOUBLE_TYPE:
      m = match_double(arg,(int) strlen(arg));
      break;
    default:
      m = (arg[0] != OPTION_CHAR);
      if (m)
        push_string(arg,(int) strlen(arg));
      break;
  }
  return (m);
}
    ...
    case TYPE:
      if (i+1 >= def->len || def->unit[i+1]->label == WHITE)
        { switch (def->unit[i]->def_val.type)
          { case STRING_TYPE:
              push_string(arg,(int) strlen(arg));
              if (match_option_suffix(def,i+1,arg+((int) strlen(arg)),argv))
                return (1);
              pop_type_match();
              return (0);
            case INT_TYPE:
              if (match_int(arg,(int) strlen(arg)))
                { if (match_option_suffix(def,i+1,arg+((int) strlen(arg)),argv))
                    return (1);
                  pop_type_match();
                }
              return (0);
            case DOUBLE_TYPE:
              if (match_double(arg,(int) strlen(arg)))
                { if (match_option_suffix(def,i+1,arg+((int) strlen(arg)),argv))
                    return (1);
                  pop_type_match();
                }
...
{ int       *has_a_match = (int *)
                             mylib::Guarded_Malloc(sizeof(int)*2*((size_t) ArgC),"Process_Arguments");
  int       *chain_cover = has_a_match + ArgC;
  Alpha_Set *all_flags   = (Alpha_Set *)
                             mylib::Guarded_Malloc(sizeof(Alpha_Set)*((size_t) ArgC),"Process_Arguments");
...
{ int i, k, v;
  ...
  Last_Req     = 0;
  A_Match      = (Match_State *)
                 mylib::Guarded_Malloc(sizeof(Match_State)*((size_t) ArgC),"Process_Arguments");
  F_Path       = (Unit **)
                 mylib::Guarded_Malloc(sizeof(Unit *)*((size_t) (ArgC+mach->longest)),"Process_Arguments");
  Match_Vector = (Match_State *)
                 mylib::Guarded_Malloc(sizeof(Match_State)*((size_t) ArgC),"Process_Arguments");
  Fstack       = (Unit **)
                 mylib::Guarded_Malloc(sizeof(Unit *)*((size_t) (ArgC+mach->longest)),"Process_Arguments");
  ...
  for (i = 1; i < argc; i++)
    { Unit *ins = A_Match[i].inst;
      ...
      if (A_Match[i].matched == PART_MATCH)
        ...
        { Match *mat = (Match *) mylib::Guarded_Malloc(sizeof(Match),"Process_Arguments");
          mat->value = Vtop;
          if (ins->kind == REQUIRED)
...
{ static int   First_Call = 1;
  ...
  { int   lines;
    ...
    if (lines == 0)
      ...
      { Master_Spec = (char **)
                      mylib::Guarded_Malloc(sizeof(char *)*((size_t) (lines+1)),"Process_Arguments");
        Master_Spec[lines] = NULL;
        while (--lines >= 0)
          { int   len = (int) (strlen(spec[lines])+2);
            char *pad = (char*) mylib::Guarded_Malloc(sizeof(char)*((size_t) len),"Process_Arguments");
...
void mylib::Print_Argument_Usage(FILE *file, int no_escapes)
{ int   i, idnt;
  char *s;
