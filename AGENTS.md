...
- Do not hard-code control flow that depends on parsing natural-language text returned by an LLM (assistant output). This is unreliable across models/providers/languages and will regress silently. Prefer structured signals: tool calls, typed schemas, explicit function arguments, status codes, durable session state, or machine-readable metadata.
