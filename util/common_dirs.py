...
def static_deploy_folder() -> str:
    return os.path.join(
        os.path.expanduser("~"), "Dropbox", "code", "my", "proxy", "static"
    )
...
def is_my_computer() -> bool:
    return (
        ...
        and os.path.exists(os.path.join(static_deploy_folder(), "atlas_test_data"))
    )
...
def vc_redist_dir() -> str:
    ...
    vc_redist_version_filename = os.path.join(
        vs_install_dir(),
        "VC",
        "Auxiliary",
        "Build",
...
def get_package_top_level_folder(file: str):
    ...
    if file.lower().endswith(".zip"):
        with zipfile.ZipFile(file, mode="r") as zf:
            ...
            res = os.path.commonpath(
                [
                    ...
                    if not nm.endswith("/") and not nm.startswith("__MACOSX")
                ]
            )
    elif (
        file.lower().endswith(".tar.gz")
        or file.lower().endswith(".tar.bz2")
        or file.lower().endswith(".tar.xz")
        ...
        cp = subprocess.run(
            [get_7za_binary(), "l", "-slt", "-sccUTF-8", file],
...
def unpack_file_to_folder(file: str, folder: str):
    ...
    if file.lower().endswith(".zip"):
        ...
        subprocess.run(
            [get_7za_binary(), "x", "-y", "-o" + folder, file], shell=False, check=True
        )


def unpack_tool_to_target_dir(
    tool_package_folder: str,
    tool_package_glob_name: str,
    tool_folder_glob_name=None,
    *,
    target_dir: str = ext_build_dir(),
) -> str:
    if tool_folder_glob_name is None:
        tool_folder_glob_name = tool_package_glob_name
    package_name = find_src_package_with_glob(
        os.path.join(tool_package_folder, tool_package_glob_name)
    )
    package_folder = get_package_top_level_folder(package_name)
    if not package_folder:
        package_unpack_folder = os.path.join(
            target_dir, os.path.splitext(os.path.basename(package_name))[0]
        )
    else:
    ...
    if not os.path.exists(package_unpack_folder):
        ...
        unpack_file_to_folder(
            package_name, target_dir if package_folder else package_unpack_folder
        )
...
def install_ninja():
    ...
    if os.path.exists(os.path.join(target_dir, "ninja")) or os.path.exists(
        os.path.join(target_dir, "ninja.exe")
    ):
        return
    if is_windows():
        unpack_file_to_folder(
            os.path.join(src_package_dir(), "ninja-win.zip"), target_dir
        )
    elif is_linux():
        unpack_file_to_folder(
            os.path.join(src_package_dir(), "ninja-linux.zip"), target_dir
        )
        os.chmod(
            os.path.join(target_dir, "ninja"),
            stat.S_IRWXU or stat.S_IXGRP or stat.S_IRGRP or stat.S_IROTH,
        )
    else:
        unpack_file_to_folder(
            os.path.join(src_package_dir(), "ninja-mac.zip"), target_dir
        )
        os.chmod(
            os.path.join(target_dir, "ninja"),
            stat.S_IRWXU or stat.S_IXGRP or stat.S_IRGRP or stat.S_IROTH,
...
def install_ffmpeg():
    if is_windows():
        ...
        os.rename(
            ...
            os.path.join(ext_build_dir(), "ffmpeg-intel"),
        )
        unpack_tool_to_target_dir(src_package_dir(), "ffmpeg*arm*")
        os.rename(
            os.path.join(ext_build_dir(), "ffmpeg"),
            os.path.join(ext_build_dir(), "ffmpeg-arm"),
        )
        subprocess.run(
            [
                "lipo",
        ...
        subprocess.run(
            ["xattr", "-cr", os.path.join(ext_build_dir(), "ffmpeg")],
            cwd=ext_build_dir(),
            shell=False,
            check=True,
        )
...
def get_cmake_binary() -> str:
    if is_windows():
        cmake_folder = find_src_package_with_glob(
            os.path.join(ext_build_dir(), "cmake-*windows*")
        )
        return os.path.join(cmake_folder, "bin", "cmake")
    elif is_linux():
        cmake_folder = find_src_package_with_glob(
            os.path.join(ext_build_dir(), "cmake-*linux*_64")
        )
        return os.path.join(cmake_folder, "bin", "cmake")
    else:
        cmake_folder = find_src_package_with_glob(
            os.path.join(ext_build_dir(), "cmake-*macos*")
        )
...
def get_ffmpeg_binary() -> str:
    if is_windows():
        folder = find_src_package_with_glob(
            os.path.join(ext_build_dir(), "ffmpeg*win*")
        )
        return os.path.join(folder, "bin", "ffmpeg.exe")
    elif is_linux():
        folder = find_src_package_with_glob(
            os.path.join(ext_build_dir(), "ffmpeg*linux*")
        )
...
def get_gperf_dir() -> str:
    if is_windows():
        folder = find_src_package_with_glob(
            os.path.join(ext_build_dir(), "gperf*-bin*")
        )
