...
#else
  chColor = texture1D(transfer_function_{*1*}, texture2D(volume_{*1*}, texCoord0).r);
#endif
  if (chColor.a > 0.0) {
...
def generate_image2d_with_transfun_frag(file: str, max_num_volumes: int):
    """
...
if __name__ == "__main__":
    if len(sys.argv) > 2:
        generate_image2d_with_transfun_frag(sys.argv[1], sys.argv[2])
    else:
        generate_image2d_with_transfun_frag(common_dirs.atlas_dir() + '/Resources/shader/image2d_with_transfun.frag',
                                            20)
