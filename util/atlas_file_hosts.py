...
_DOTENV_KEYS: Final[frozenset[str]] = frozenset({_PRIMARY_ENV_VAR, _BACKUP_ENV_VAR})
...
def get_file_hosts() -> tuple[str, str]:
    """Return (primary_host, backup_host) for Atlas downloads.

    Values are sourced from the process env and optionally repo-root `.env` /
    `.env.local` (via `atlas_env.load_repo_dotenv`). Each host should be an
...
def with_static_urls(
    ...
    *,
    ...
    """Return `raw_files` with `url` and `backup_url` filled in.
